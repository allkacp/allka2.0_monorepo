...
// Mesma regra adotada para produtos/níveis de parceiro: exclusão física
// exige permissão administrativa explícita (module "sistema", action
// "delete") — ver comentário equivalente em routes/products.ts.
router.delete(
  "/:id",
  verifyToken,
  requireRole("admin"),
  ...
  async (req, res, next) => {
    try {
      ...
      await prisma.nomadeLevel.delete({ where: { id } });
      res.status(204).send();
    } catch (err) {
      next(err);
    }
