/**
 * seed-realistic-demo.ts (substitui src/prod-scripts/seed-real.ts)
 * ================================================================
 * Seed de DEMONSTRAÇÃO — Agency / Empresa / Líder realistas para uso
 * exclusivamente LOCAL. Isto NÃO é uma ferramenta de produção: cria
 * usuários, empresas, projetos e tarefas de demonstração com senha de
 * teste. Nunca deve rodar fora de localhost/127.0.0.1.
 *
 *   - usa o client Prisma compartilhado (src/lib/prisma);
 *   - usa withProjectCode (src/lib/create-project) — mesma transação da
 *     sequência, nunca reimplementa getNextSequenceValue;
 *   - usa confirmPaymentAndGenerateProjectTasks (src/lib/confirm-payment)
 *     pro projeto do líder — nunca cria Payment PAGO na mão;
 *   - usa generateTasksFromSpec (src/lib/generate-tasks-from-spec) pras
 *     tarefas do líder — nunca projectTask.create/update direto, create-only
 *     em reexecuções (preserva status/responsáveis/datas/etapas).
 *
 * Fica em src/scripts/ DE PROPÓSITO: essa pasta é excluída do build
 * (tsconfig "exclude") — este script NUNCA deve existir em dist/, nunca é
 * citado no Dockerfile nem no workflow de deploy, e não pode ser invocado
 * dentro do container de produção (não há tsx lá, e o arquivo nem estaria
 * presente na imagem).
 *
 * Guarda de ambiente: bloqueia ANTES de qualquer consulta ao Prisma se o
 * host de DATABASE_URL não for exatamente "localhost" ou "127.0.0.1" —
 * independente de NODE_ENV. Não existe "modo produção autorizado" aqui.
 *
 * Credenciais: exige SEED_TEST_USER_PASSWORD no ambiente (sem fallback
 * embutido) — encerra com erro antes de qualquer escrita se ausente. Nunca
 * altera password_hash/is_active de usuário já existente (só define isso na
 * criação); nunca imprime a senha.
 *
 * Uso: npm run db:seed:demo-realistic
 * ================================================================
 */
import "dotenv/config";
import bcrypt from "bcryptjs";

// ─── Guarda de ambiente — roda ANTES de importar/consultar o Prisma ───────
// Checagem por string pura (new URL não faz DNS) — bloqueia sem nunca abrir
// conexão de rede quando o host não é local.
function assertLocalDatabaseOrExit(): void {
  ...
  try {
    ...
    console.error("❌ BLOQUEADO: DATABASE_URL ausente ou inválida — impossível confirmar que o host é local.");
    process.exit(1);
  }
  const isLocal = hostname === "localhost" || hostname === "127.0.0.1";
  if (!isLocal) {
    console.error(
      `❌ BLOQUEADO: host "${hostname}" não é localhost/127.0.0.1. ` +
        "seed-realistic-demo.ts é um seed de DEMONSTRAÇÃO — nunca roda contra host remoto (rede local, QA, staging ou produção), independente de NODE_ENV.",
    );
    process.exit(1);
  }
  console.log(`── Ambiente: host=${hostname} (local, autorizado) ──`);
}
assertLocalDatabaseOrExit();

const SEED_PASSWORD = process.env.SEED_TEST_USER_PASSWORD;
if (!SEED_PASSWORD) {
  console.error("❌ BLOQUEADO: SEED_TEST_USER_PASSWORD não definida — obrigatória, sem valor padrão embutido.");
  process.exit(1);
}
...
import { generateTasksFromSpec, type TaskSpec } from "../lib/generate-tasks-from-spec";
...
const STAGE_TITLES = [
  { code: "E01", titulo: "Briefing & Acessos", descricao: "Coleta de briefing e validação de acessos iniciais." },
  { code: "E02", titulo: "Configuração & Setup", descricao: "Estruturação de campanhas e configuração técnica." },
  { code: "E03", titulo: "Execução & Otimização", descricao: "Veiculação, monitoramento e otimização contínua." },
  { code: "E04", titulo: "Entrega & Validação", descricao: "Relatório final e validação de resultados." },
];
...
async function seedBaseUsers() {
  ...
  // update: {} de propósito em todos os upserts de usuário abaixo — usuário
  // já existente nunca tem password_hash/is_active tocados por este seed.
  await prisma.user.upsert({
    where: { email: "cp@lamego.com.vc" },
    update: {},
    create: { email: "cp@lamego.com.vc", password_hash: hash, name: "Vinicius Guardia", role: "admin", account_type: "admin", is_active: true },
  });

  await prisma.user.upsert({
    where: { email: "agencia@allka.com.vc" },
    update: {},
    create: { email: "agencia@allka.com.vc", password_hash: hash, name: "Lamego Teste Agency", role: "agency_admin", account_type: "agencias", is_active: true },
  });

  await prisma.user.upsert({
    where: { email: "empresa@allka.com.vc" },
    update: {},
    create: { email: "empresa@allka.com.vc", password_hash: hash, name: "TechStart Soluções Digitais", role: "company_admin", account_type: "empresas", is_active: true },
  });
  ...
  await prisma.company.upsert({
    ...
    update: {},
    create: { id: CT_COMPANY_ID, name: "Company Test", cnpj: "00.000.001/0001-99", email: "company@allka.test", phone: "(11) 90000-0001", status: "ativo", segment: "Tecnologia" },
  });
  await prisma.user.upsert({
    where: { email: "company@allka.test" },
    // company_id não é sensível (é vínculo estrutural, não credencial) —
    // seguro sincronizar mesmo em usuário existente.
...
async function seedAgencia() {
  ...
  const userRecord = await prisma.user.findFirst({ where: { email: AGENCY_EMAIL }, select: { id: true } });
  if (!userRecord) {
    console.warn(`[seed-realistic-demo] agency: usuário ${AGENCY_EMAIL} não existe — pulando.`);
    return;
  }
  ...
  for (const p of projects) {
    const { id, status, progress, spent, ...rest } = p;
    await withProjectCode(prisma, (tx, projectCode) =>
      tx.project.upsert({
        where: { id },
        update: { status, progress, spent },
        create: {
          id,
          ...rest,
          status,
          ...
          portfolio_permission: status === "completed",
          bitrix_sync: false,
          from_lead: false,
          nomades: "[]",
  ...
  // Tarefas operacionais leves (TaskExecution) — modelo separado de
  // ProjectTask, não sujeito ao gate de Payment/PaymentItem.
  const tasks = [
  ...
  for (const inv of invoices) {
    await prisma.invoice.upsert({ where: { id: inv.id }, update: {}, create: { ...inv, paid_at: inv.paid_at ?? undefined } });
  }

  console.log(`[seed-realistic-demo] agency OK: ${agency.name} (${clients.length} clientes, ${projects.length} projetos, ${tasks.length} tarefas, ${invoices.length} faturas)`);
}
...
async function seedEmpresa() {
  ...
  const empresaUser = await prisma.user.findFirst({ where: { email: EMPRESA_EMAIL, account_type: "empresas" }, select: { id: true } });
  ...
  await prisma.user.updateMany({ where: { email: EMPRESA_EMAIL, account_type: "empresas" }, data: { company_id: COMPANY_ID } });
  ...
  for (const p of projects) {
    const { id, status, progress, ...rest } = p;
    await withProjectCode(prisma, (tx, projectCode) =>
      tx.project.upsert({
        where: { id },
        update: { status, progress },
        create: { id, ...rest, status, progress, client_id: COMPANY_ID, created_by_user_id: empresaUser?.id ?? null, project_code: projectCode },
      }),
    );
  }
  ...
  console.log(`[seed-realistic-demo] empresa OK: ${company.name} (${projects.length} projetos, ${tasks.length} tarefas, ${invoices.length} faturas)`);
}
...
async function seedLider() {
  ...
  let liderUser =
    ...
    (await prisma.user.findFirst({ where: { OR: [{ account_type: "lider" }, { role: "lider" }] }, select: { id: true, email: true } }));

  if (liderUser) {
    console.log(`[seed-realistic-demo] lider: usando usuário ${liderUser.email}`);
  } else {
    liderUser = await prisma.user.upsert({
      where: { id: LIDER_USER_ID },
      update: {},
      create: { id: LIDER_USER_ID, email: LIDER_EMAIL, password_hash: hash, name: "Líder Performance Seed", role: "lider", account_type: "lider", is_active: true },
      select: { id: true, email: true },
    });
    console.log(`[seed-realistic-demo] lider: usuário seed criado ${liderUser.email}`);
  }
  ...
  for (const n of nomadeData) {
    ...
    const nomade = await prisma.nomade.upsert({
      ...
      create: {
        ...
        is_leader: false,
        performance_avg_rating: 4.5,
        performance_on_time: 0.9,
  ...
  await prisma.product.upsert({
    ...
    update: {},
    // is_active:false de propósito — este produto é um fixture 100% autoral
    // sem CatalogTask real por trás (ver generateTasksFromSpec); mantê-lo
    // fora das varreduras de "produto ativo sem modelo" (db:seed:catalog-tasks)
    // evita que um CatalogTask provisório gere uma 16ª tarefa genérica
    // indesejada quando o pagamento for confirmado abaixo.
    create: { id: "seed-product-perf-01", name: "Gestão de Performance", description: "Gestão de campanhas de performance e mídia paga", category: CATEGORY, base_price: 2500, complexity: "intermediate", is_active: false },
  });
  ...
  await withProjectCode(prisma, (tx, projectCode) =>
    tx.project.upsert({
      ...
      create: {
        id: "seed-project-lider-01",
        title: "Projeto Allka Seed",
        description: "Gestão de performance e mídia paga",
        client_id: "seed-company-lider-01",
        status: "in-progress",
        lifecycle: "mensal",
        ...
        consultant_email: LIDER_EMAIL,
        start_date: past(30),
        created_by_user_id: liderUserId,
        project_code: projectCode,
      },
    }),
  );
  ...
  // Payment PAGO + PaymentItem reais, pelo serviço central — nunca criados
  // na mão. Nenhuma ProjectTask nasce sem isto por trás.
  ...
  const paymentResult = await withIdempotentRetry(() =>
    prisma.$transaction((tx) =>
      confirmPaymentAndGenerateProjectTasks(tx, {
        projectId: "seed-project-lider-01",
  ...
  const TASKS = [
    { code: "T-SEED-001", title: "Criar campanha Google Ads — Q2", status: "PARA_LANCAMENTO", due_date: future(7), nomadeIdx: 0, priority: "high" },
    { code: "T-SEED-002", title: "Otimizar campanhas de remarketing", status: "PARA_LANCAMENTO", due_date: future(10), nomadeIdx: 1, priority: "medium" },
    { code: "T-SEED-003", title: "Análise de palavras-chave SEO", status: "PARA_LANCAMENTO", due_date: future(5), nomadeIdx: 2, priority: "medium" },
    { code: "T-SEED-004", title: "Relatório mensal de performance", status: "EM_EXECUCAO", due_date: future(3), nomadeIdx: 0, priority: "high" },
    { code: "T-SEED-005", title: "Configuração de pixel de conversão", status: "EM_EXECUCAO", due_date: future(4), nomadeIdx: 1, priority: "urgent" },
    { code: "T-SEED-006", title: "Criação de públicos personalizados", status: "EM_EXECUCAO", due_date: future(6), nomadeIdx: 2, priority: "medium" },
    { code: "T-SEED-007", title: "Briefing campanha Black Friday", status: "LANCAMENTO_ENVIADO_PARA_ANALISE", due_date: future(2), nomadeIdx: 0, priority: "urgent" },
    { code: "T-SEED-008", title: "Briefing campanha institucional", status: "LANCAMENTO_ENVIADO_PARA_ANALISE", due_date: future(1), nomadeIdx: 1, priority: "medium" },
    { code: "T-SEED-009", title: "Entrega relatório de impressões", status: "ENTREGA_PENDENTE", due_date: past(1), nomadeIdx: 2, priority: "high" },
    { code: "T-SEED-010", title: "Entrega de criativos para Meta Ads", status: "ENTREGA_PENDENTE", due_date: past(2), nomadeIdx: 0, priority: "medium" },
    { code: "T-SEED-011", title: "Ajuste de lances por horário", status: "ENTREGA_ATRASADA", due_date: past(5), nomadeIdx: 1, priority: "high" },
    { code: "T-SEED-012", title: "Atualização de estratégia de lances", status: "ENTREGA_ATRASADA", due_date: past(8), nomadeIdx: 2, priority: "urgent" },
    { code: "T-SEED-013", title: "Revisão de copy dos anúncios", status: "REPROVADA", due_date: past(3), nomadeIdx: 0, priority: "medium" },
    { code: "T-SEED-014", title: "Reestruturação de grupos de anúncio", status: "REPROVADA", due_date: past(6), nomadeIdx: 1, priority: "high" },
    { code: "T-SEED-015", title: "Campanha sazonal aprovada", status: "APROVADA", due_date: past(1), nomadeIdx: 2, priority: "medium" },
  ];
  ...
  const taskSpecs: TaskSpec[] = TASKS.map((t) => {
    ...
    return {
      ...
      dueDate: t.due_date,
      fase: "Execução",
      liderResponsavelId: liderUserId,
      responsavelAgenciaId: "seed-agencia-user-01",
      nomadeResponsavelId: nomadeIds[t.nomadeIdx],
      stages: STAGE_TITLES.map((s, i) => ({
        code: s.code,
        catalogStepRef: s.code,
        titulo: s.titulo,
  ...
  const genResult = await withIdempotentRetry(() =>
    prisma.$transaction((tx) =>
      generateTasksFromSpec(tx, {
        ...
        projectProductId: "seed-pp-lider-01",
        productId: "seed-product-perf-01",
        tasks: taskSpecs,
        syncMetadata: true,
      }),
    ),
  ...
  console.log(
    `[seed-realistic-demo] lider OK: ${AREA} (${nomadeData.length} nômades, ${genResult.created} tarefas criadas, ` +
      `${genResult.reused} reaproveitadas, etapas: ${genResult.stagesCreated} criadas / ${genResult.stagesReused} reaproveitadas)`,
  );
}

// ── Runner ───────────────────────────────────────────────────────────────────
async function main() {
  console.log("[seed-realistic-demo] iniciando seed de demonstração local (upsert idempotente, create-only)…");
  const steps: Array<[string, () => Promise<void>]> = [
    ["base-users", seedBaseUsers],
    ["agencia", seedAgencia],
    ["empresa", seedEmpresa],
    ["lider", seedLider],
  ];
  ...
  for (const [name, fn] of steps) {
    try {
      ...
      console.error(`[seed-realistic-demo] erro no bloco ${name}:`, msg);
      errors.push(`${name}: ${msg}`);
    }
  }
  console.log("[seed-realistic-demo] concluído.");
  if (errors.length > 0) {
    console.error(`[seed-realistic-demo] ${errors.length} bloco(s) com erro — ver acima.`);
    process.exitCode = 1;
  }
}
...
main()
  .catch((e) => {
    console.error("[seed-realistic-demo] falha fatal:", e);
    process.exitCode = 1;
  })
  .finally(async () => {
