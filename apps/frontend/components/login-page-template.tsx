...
const UI: Record<
  Locale,
  {
    ...
    errorFill: string;
    errorLogin: string;
    errorForbidden: (panel: string) => string;
    errorNetwork: string;
    socialSoon: string;
  }
> = {
  pt: {
    ...
    errorForbidden: (panel) =>
      `Este usuário não possui acesso ao painel ${panel}.`,
    errorNetwork:
  ...
  en: {
    ...
    errorForbidden: (panel) =>
      `This user does not have access to the ${panel} panel.`,
  ...
  es: {
    ...
    errorForbidden: (panel) =>
      `Este usuario no tiene acceso al panel ${panel}.`,
  ...
  zh: {
    ...
    errorFill: "请填写邮箱和密码。",
    errorLogin: "邮箱或密码不正确。",
    errorForbidden: (panel) => `该用户无权访问 ${panel} 面板。`,
...
export function LoginPageTemplate({ config }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res: any = await apiClient.login(
        email,
        password,
        config.accessType,
      );
      if (res?.token) {
        ...
        localStorage.setItem("allka_last_access_type", config.accessType);
        if (res.user)
      ...
      if (msg === "ACCESS_FORBIDDEN") {
        const label =
          ACCESS_TYPES.find((a) => a.id === config.accessType)?.label ??
          config.accessType;
        setErrorMsg(ui.errorForbidden(label));
      } else if (
        msg === "Failed to fetch" ||
        msg.includes("fetch") ||
