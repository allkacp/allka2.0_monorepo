...
export function ProductCatalogView({
  ...
  const allRecurrences = useMemo(
    () =>
      Array.from(
        new Set(
          activeProducts.map((p) => (p as any).recurrence).filter(Boolean),
        ),
      ),
    [activeProducts],
  );
  ...
  const filteredProducts = useMemo(() => {
    let list = activeProducts.filter((p) => {
      ...
      const matchesRecurrence =
        ...
        (p as any).recurrence === recurrenceFilter;
      const hasVariations =
        (p.variations || []).filter((v: any) => v.isActive !== false).length >
        0;
    ...
    list = [...list].sort((a, b) => {
      if (sort === "smart") {
        const ra = getRating(a.id),
          rb = getRating(b.id);
        if (rb.totalDeliveries !== ra.totalDeliveries)
          return rb.totalDeliveries - ra.totalDeliveries;
      ...
      if (sort === "rating_desc") {
        ...
        if (rb.rating !== ra.rating) return rb.rating - ra.rating;
        return (
          rb.totalDeliveries - ra.totalDeliveries ||
  ...
  const currentVariationsLabel =
    VARIATIONS_OPTIONS.find((o) => o.value === variationsFilter)?.label ??
    "Variações";
  ...
  if (loading) return <PageLoader text="Carregando catálogo de produtos…" />;

  if (productsError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[340px] gap-4 text-center px-6">
            ...
            <DropdownMenuContent align="end" className="w-48">
              {SORT_OPTIONS.map((o) => (
                ...
                >
                  ...
                  {sort !== o.value && (
                    <span className="w-5 mr-0.5 inline-block" />
                  )}
                  {o.label}
                </DropdownMenuItem>
              ))}
          ...
          {[
            {
              ...
              label: "Mais vendidos",
              Icon: TrendingUp,
              isActive: sort === "deliveries_desc",
              onClick: () =>
                setSort(
                  sort === "deliveries_desc" ? "smart" : "deliveries_desc",
                ),
              colorActive: "bg-emerald-500 border-emerald-500 text-white",
            },
            {
              id: "top_rated",
              label: "Melhor avaliados",
              Icon: Sparkles,
              isActive: sort === "rating_desc",
              onClick: () =>
                setSort(sort === "rating_desc" ? "smart" : "rating_desc"),
              colorActive: "bg-amber-400 border-amber-400 text-white",
            },
            {
              id: "price_low",
              label: "Menor preço",
              ...
              onClick: () =>
                setSort(sort === "price_asc" ? "smart" : "price_asc"),
              colorActive: "bg-slate-700 border-slate-700 text-white",
            },
            {
              id: "price_high",
              label: "Maior preço",
              ...
              onClick: () =>
                setSort(sort === "price_desc" ? "smart" : "price_desc"),
              colorActive: "bg-slate-700 border-slate-700 text-white",
            },
            {
              id: "sep",
            ...
            },
            {
              ...
              label: "Avulso",
              Icon: Zap,
              isActive: recurrenceFilter === "Avulso",
              onClick: () =>
                setRecurrenceFilter(
                  recurrenceFilter === "Avulso" ? "all" : "Avulso",
                ),
              colorActive: "bg-blue-500 border-blue-500 text-white",
            },
            {
              id: "mensal",
              label: "Mensal",
              Icon: RotateCcw,
              isActive: recurrenceFilter === "Mensal",
              onClick: () =>
                setRecurrenceFilter(
                  recurrenceFilter === "Mensal" ? "all" : "Mensal",
                ),
              colorActive: "bg-blue-500 border-blue-500 text-white",
            },
            {
              id: "with_vars",
              label: "Com mais opções",
              Icon: SlidersHorizontal,
              isActive: variationsFilter === "yes",
              onClick: () =>
                setVariationsFilter(variationsFilter === "yes" ? "all" : "yes"),
              colorActive: "bg-violet-500 border-violet-500 text-white",
            },
            ...
            if (id === "sep")
              return (
                <span
                  key="sep"
                  className="text-slate-200 select-none hidden sm:inline"
                >
                  |
                </span>
              );
                    ...
                    <div className="flex items-start gap-2 flex-wrap">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="top">
                          Código único do produto na plataforma
                        </TooltipContent>
                      </Tooltip>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <Tooltip>
                        ...
                        <TooltipContent side="bottom">
                          Avaliação média ponderada pelos nômades ativos
                          habilitados
                        </TooltipContent>
                      </Tooltip>
                      {activeVariations.length > 0 && (
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom">
                            Este produto possui variações — veja as opções em
                            "Detalhes"
                          </TooltipContent>
                        </Tooltip>
                      )}
                      {(product as any).deliveryDays && (
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom">
                            Prazo estimado de entrega após solicitação
                          </TooltipContent>
                        </Tooltip>
                      )}
                    </div>
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={cn(
                      "relative overflow-hidden shrink-0 bg-linear-to-br from-slate-900 via-slate-800 to-slate-900",
                      isCompact ? "h-36" : "h-48",
                    )}
                  >
                    {productImage ? (
                  ...
                  >
                    ...
                    <div>
                      <div className="flex items-start justify-between gap-1.5">
                        <h3
                          className={cn(
                            "font-bold text-slate-900 leading-snug line-clamp-2 group-hover:text-blue-600 transition-colors flex-1",
                      ...
                      {!isCompact &&
                        (product as any).tags
                          ...
                          .map((tag: string) => (
                            <Tooltip key={tag}>
                              <TooltipTrigger asChild>
                                <span className="inline-flex items-center rounded-full px-2.5 py-1 text-xs bg-violet-50 text-violet-600 font-medium border border-violet-100 cursor-default">
                                  {tag}
                                </span>
                              </TooltipTrigger>
                              <TooltipContent side="top">
                                Tag de identificação do produto
                              </TooltipContent>
                            </Tooltip>
                          ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-500">
                      ...
                      {!isCompact && product.addOns?.length > 0 && (
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom">
                            Complementos disponíveis para adicionar a este
                            produto
                          </TooltipContent>
                        </Tooltip>
                      )}
      ...
      {mode === "panel" && selectedProducts.length > 0 && onConfirm && (
        <div className="shrink-0 border-t border-slate-100 bg-white/95 backdrop-blur-sm px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              className="h-10 px-5 flex items-center gap-2 text-sm font-bold text-white rounded-xl shadow-md hover:opacity-90 active:scale-[0.98] transition-all"
              style={{
                background:
                  "var(--app-brand-button, linear-gradient(135deg, #1a2a6f 0%, #c81a7f 100%))",
              }}
