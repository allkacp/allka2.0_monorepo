...
const gradientPresets = [
  {
    ...
    value:
      "custom-gradient:linear-gradient(to bottom, rgba(0,0,0,0.7) 0%, #1a1a4f 10%, #2d1b4e 25%, #4a1a8a 40%, #5a2a9f 50%, #4a1a8a 60%, #2d1b4e 75%, #1a1a4f 90%, rgba(0,0,0,0.7) 100%)",
    preview:
      "linear-gradient(to bottom, rgba(0,0,0,0.7) 0%, #1a1a4f 10%, #2d1b4e 25%, #4a1a8a 40%, #5a2a9f 50%, #4a1a8a 60%, #2d1b4e 75%, #1a1a4f 90%, rgba(0,0,0,0.7) 100%)",
    category: "allka",
    isDefault: true,
  },
  {
    ...
    value:
      "custom-gradient:linear-gradient(to bottom right, #1e3a8a, #1e40af, #164e63)",
  ...
  },
  {
    name: "Galáxia Roxa",
    value:
      "custom-gradient:linear-gradient(to bottom right, #581c87, #5b21b6, #312e81)",
...
const backgroundPresets = [
  {
    ...
    accentColor:
      "custom-gradient:linear-gradient(135deg, #0d1438 0%, #1a1048 100%)",
  },
];
...
const DEFAULT_THEME_V1 = {
  backgroundColor:
    "custom-gradient:linear-gradient(180deg, #0f172a 0%, #1e3a8a 50%, #1e40af 100%)",
...
export function SidebarSettingsModal({
  ...
  const [confirmAction, setConfirmAction] = useState<"discard" | "save" | null>(
    null,
  );
  const [deletePresetId, setDeletePresetId] = useState<string | null>(null);
  const [deleteImagePresetId, setDeleteImagePresetId] = useState<string | null>(
    null,
  );
  ...
  const [customPresets, setCustomPresets] = useState<
    Array<{ id: string; name: string; value: string; preview: string }>
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("allka_custom_presets") ?? "[]");
    } catch {
      return [];
    }
  });
  const [customImagePresets, setCustomImagePresets] = useState<
    Array<{ id: string; name: string; url: string; accentColor: string }>
  >(() => {
    try {
      return JSON.parse(
        localStorage.getItem("allka_custom_image_presets") ?? "[]",
      );
    } catch {
      return [];
    }
  ...
  const [headerGDir, setHeaderGDir] = useState<
    "to right" | "to bottom" | "135deg"
  >("135deg");
  const [buttonG1, setButtonG1] = useState("#1a2a6f");
  const [buttonG2, setButtonG2] = useState("#c81a7f");
  const [buttonGDir, setButtonGDir] = useState<
    "to right" | "to bottom" | "135deg"
  >("135deg");
  ...
  const extractDominantColor = (url: string): Promise<string> => {
    return new Promise((resolve) => {
      const fallback =
        "custom-gradient:linear-gradient(135deg, #0f172a 0%, #1e293b 100%)";
      try {
        ...
        img.onload = () => {
          try {
            ...
            const lr = ar * scale,
              ...
              lb = ab * scale;
            resolve(
              `custom-gradient:linear-gradient(135deg, #${toHex(dr)}${toHex(dg)}${toHex(db)} 0%, #${toHex(lr)}${toHex(lg)}${toHex(lb)} 100%)`,
            );
          } catch {
            resolve(fallback);
          }
  ...
  const backgroundMode =
    (sidebarSettings.backgroundMode as
      | "gradient"
      | "image"
      | "image+gradient") || "gradient";
  const imagePosition =
    (sidebarSettings.imagePosition as "top" | "center" | "bottom") || "center";
  const imageAlignment =
    (sidebarSettings.imageAlignment as "left" | "center" | "right") || "center";
  const imageScale = sidebarSettings.imageScale ?? 100;
  const customGradientColor1 =
    sidebarSettings.customGradientColor1 || "#3b82f6";
  const customGradientColor2 =
    sidebarSettings.customGradientColor2 || "#8b5cf6";
  const customGradientDirection =
    (sidebarSettings.customGradientDirection as
      | "to right"
      | "to bottom"
      | "135deg") || "to right";
  ...
  const saveCustomPreset = () => {
    ...
    try {
      localStorage.setItem("allka_custom_presets", JSON.stringify(updated));
    } catch {}
      ...
      >
            ...
            <div className="flex items-start gap-3.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-white/65 mt-0.5 leading-snug">
                  Ajuste e preferências visuais da plataforma
                </p>
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  ...
                  <div className="flex items-end gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <label
                        className="relative flex items-center justify-center cursor-pointer overflow-hidden rounded-lg border-2 border-dashed border-slate-200 bg-slate-50 hover:border-blue-400 hover:bg-blue-50/40 transition-colors"
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {presets.map((preset) => {
                      const active =
                        selectedColor === preset.theme.backgroundColor;
                      return (
                        <button
                          key={preset.name}
                          type="button"
                          onClick={() =>
                            applyFullTheme(
                              preset.theme as typeof sidebarSettings,
                            )
                          }
                  ...
                  <div className="flex items-center gap-2 w-fit">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        applyPreview({
                          customGradientColor1: c1,
                          backgroundColor: `custom-gradient:linear-gradient(${customGradientDirection}, ${c1}, ${customGradientColor2})`,
                        });
                      }}
                    ...
                    />
                    <div
                      className="w-28 h-8 rounded-lg border border-slate-200 shadow-inner shrink-0"
                      style={{
                        background: `linear-gradient(${customGradientDirection}, ${customGradientColor1}, ${customGradientColor2})`,
                      }}
                    />
                    <input
                      type="color"
                      ...
                      onChange={(e) => {
                        ...
                        applyPreview({
                          customGradientColor2: c2,
                          backgroundColor: `custom-gradient:linear-gradient(${customGradientDirection}, ${customGradientColor1}, ${c2})`,
                        });
                      }}
                      className="w-8 h-8 rounded-lg cursor-pointer border border-slate-200 p-0.5 shrink-0"
                      title="Cor final"
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const dir = e.target
                          .value as typeof customGradientDirection;
                        applyPreview({
                          customGradientDirection: dir,
                          backgroundColor: `custom-gradient:linear-gradient(${dir}, ${customGradientColor1}, ${customGradientColor2})`,
                        });
                      }}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const cg = `custom-gradient:linear-gradient(${customGradientDirection}, ${customGradientColor1}, ${customGradientColor2})`;
                        applyPreset(cg);
                  ...
                  <div className="flex gap-1 mb-2">
                    {(
                      [
                        ...
                        { value: "image", label: "Só Imagem" },
                        { value: "image+gradient", label: "Img + Sombra" },
                      ] as const
                    ).map((mode) => (
                      <button
                        key={mode.value}
                        ...
                        onClick={() =>
                          applyPreview({
                            backgroundMode: mode.value as typeof backgroundMode,
                          })
                        }
                  ...
                  {backgroundMode !== "gradient" && (
                    <div className="space-y-2">
                      <label
                        ...
                        className="flex items-center gap-2 border border-dashed border-slate-300 rounded-lg px-2.5 py-1.5 cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-colors"
                      >
                        ...
                        <div className="rounded-lg border border-slate-200 overflow-hidden">
                          <div className="relative">
                            ...
                            <div className="absolute top-1 right-1 flex gap-1">
                              <button
                                type="button"
                                onClick={() => setSaveImagePresetOpen(true)}
                          ...
                          <div className="p-2 bg-slate-50 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[10px]">
                            ...
                            <div className="col-span-2 flex items-center gap-2">
                              ...
                              <input
                                ...
                                className="flex-1 h-1 rounded accent-blue-500"
                              />
                            </div>
                            <div className="col-span-2 flex items-center gap-2">
                              <span className="text-slate-400 w-20 shrink-0">
                                Opacidade {imageOpacity}%
                              </span>
                              <input
                                type="range"
                                min="0"
                        ...
                        <div className="flex gap-1.5 overflow-x-auto pb-1">
                          {[
                            ...
                            >
                              ...
                              <p className="text-[9px] text-slate-500 text-center mt-0.5 truncate w-14">
                                {preset.name}
                              </p>
                              {"isCustom" in preset && preset.isCustom && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    setDeleteImagePresetId(preset.id as string)
                                  }
                    ...
                    <div className="space-y-1.5">
                      <div className="flex flex-wrap gap-1">
                        {[
                          ...presets.map((p) => ({
                            name: p.name,
                            value: p.theme.backgroundColor,
                            preview: buildCSSPreview(p.theme.backgroundColor),
                          })),
                          ...gradientPresets.map((g) => ({
                            name: g.name,
                            value: g.value,
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              applyPreview({ headerBg: item.value })
                            }
                      ...
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          value={headerG1}
                          onChange={(e) => {
                            setHeaderG1(e.target.value);
                            applyPreview({
                              headerBg: `custom-gradient:linear-gradient(${headerGDir}, ${e.target.value}, ${headerG2})`,
                            });
                          }}
                          className="w-6 h-6 rounded cursor-pointer border border-slate-200 p-0.5 shrink-0"
                        />
                        <div
                          className="flex-1 h-6 rounded border border-slate-200"
                          style={{
                            background: `linear-gradient(${headerGDir}, ${headerG1}, ${headerG2})`,
                          }}
                        />
                        <input
                          type="color"
                          value={headerG2}
                          onChange={(e) => {
                            setHeaderG2(e.target.value);
                            applyPreview({
                              headerBg: `custom-gradient:linear-gradient(${headerGDir}, ${headerG1}, ${e.target.value})`,
                            });
                          }}
                          className="w-6 h-6 rounded cursor-pointer border border-slate-200 p-0.5 shrink-0"
                        />
                        <select
                          value={headerGDir}
                          onChange={(e) => {
                            const d = e.target.value as typeof headerGDir;
                            setHeaderGDir(d);
                            applyPreview({
                              headerBg: `custom-gradient:linear-gradient(${d}, ${headerG1}, ${headerG2})`,
                            });
                          }}
                          className="h-6 px-1 border border-slate-200 rounded text-[11px] bg-white shrink-0"
                        >
                          <option value="to right">→</option>
                          <option value="to bottom">↓</option>
                          <option value="135deg">↘</option>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex flex-wrap gap-1">
                        {solidColors.map((color) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              applyPreview({ buttonBg: color.value })
                            }
                            title={color.name}
                            className={cn(
                              "relative w-5 h-5 rounded border transition-all hover:scale-110",
                              sidebarSettings.buttonBg === color.value
                                ? "ring-2 ring-blue-500 ring-offset-1 border-transparent"
                                : "border-black/10",
                            )}
                            style={{ backgroundColor: color.preview }}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {[
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              applyPreview({ buttonBg: item.value })
                            }
                      ...
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          value={buttonG1}
                          onChange={(e) => {
                            setButtonG1(e.target.value);
                            applyPreview({
                              buttonBg: `custom-gradient:linear-gradient(${buttonGDir}, ${e.target.value}, ${buttonG2})`,
                            });
                          }}
                          className="w-6 h-6 rounded cursor-pointer border border-slate-200 p-0.5 shrink-0"
                        />
                        <div
                          className="flex-1 h-6 rounded border border-slate-200"
                          style={{
                            background: `linear-gradient(${buttonGDir}, ${buttonG1}, ${buttonG2})`,
                          }}
                        />
                        <input
                          type="color"
                          value={buttonG2}
                          onChange={(e) => {
                            setButtonG2(e.target.value);
                            applyPreview({
                              buttonBg: `custom-gradient:linear-gradient(${buttonGDir}, ${buttonG1}, ${e.target.value})`,
                            });
                          }}
                          className="w-6 h-6 rounded cursor-pointer border border-slate-200 p-0.5 shrink-0"
                        />
                        <select
                          value={buttonGDir}
                          onChange={(e) => {
                            const d = e.target.value as typeof buttonGDir;
                            setButtonGDir(d);
                            applyPreview({
                              buttonBg: `custom-gradient:linear-gradient(${d}, ${buttonG1}, ${buttonG2})`,
                            });
                          }}
                          className="h-6 px-1 border border-slate-200 rounded text-[11px] bg-white shrink-0"
                        >
                      ...
                      <button
                        type="button"
                        className="btn-brand w-full h-7 rounded-lg text-xs font-semibold text-white pointer-events-none"
                      >
                        Prévia do botão
                      </button>
                    </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-2.5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      className={`h-5 px-2 rounded-full border text-[10px] font-medium transition-all ${
                        !sidebarSettings.activeItemColor
                          ? "border-emerald-400/40 bg-emerald-50 text-emerald-700"
                          : "border-slate-200 bg-slate-50 text-slate-500"
                      }`}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        applyPreview({ activeItemColor: e.target.value })
                      }
                    ...
                    <div className="flex-1 h-8 rounded border border-slate-200 flex items-center px-2 gap-2 text-xs text-slate-600">
                      <div
                        ...
                        style={{
                          backgroundColor:
                            sidebarSettings.activeItemColor || "#c81a7f",
                        }}
                      />
                  ...
                  <p className="text-[10px] text-slate-400 mt-1.5">
                    Cor do fundo e do badge do item selecionado no menu.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-[11px] text-slate-500 leading-relaxed">
                      O tamanho global da fonte é controlado pelos botões{" "}
                      <span className="font-semibold text-slate-700">
                        A- / A / A+
                      </span>{" "}
                      no cabeçalho da plataforma. A preferência fica salva
                      automaticamente.
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white divide-y divide-slate-100">
                  ...
                  {(
                    ...
                    >
                      <span className="text-[11px] font-medium text-slate-600 w-24 shrink-0">
                        {label}
                      </span>
                      <div className="flex gap-1 flex-1">
                        {opts.map(({ v, l }) => (
                          <button
                            key={v}
                            type="button"
                            onClick={() => applyPreview({ [key]: v })}
                            className={`flex-1 py-1 rounded-md text-[11px] font-medium border transition-all ${sidebarSettings[key] === v ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 bg-slate-50 text-slate-600 hover:border-slate-300"}`}
                          >
                            {l}
                          </button>
                        ))}
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-2.5">
                  ...
                  <div className="space-y-1.5 text-slate-700">
                    <p
                      style={{
                        fontSize: "var(--app-font-heading, 1rem)",
                        fontWeight: 600,
                        lineHeight: 1.3,
                      }}
                    >
                    ...
                    <p
                      style={{
                        fontSize: "var(--app-font-body, 0.8125rem)",
                        color: "#64748b",
                      }}
                    >
                      Texto de corpo normal, descrições e labels.
                    </p>
                        ...
                        <thead className="bg-slate-50">
                          <tr>
                            <th
                              ...
                              style={{
                                fontSize: "var(--app-font-table, 0.7rem)",
                                fontWeight: 600,
                              }}
                            >
                              Coluna A
        ...
        <footer className="border-t px-4 py-2.5 flex gap-2 bg-gray-50 flex-shrink-0">
          ...
          <Button
            ...
            onClick={handleSave}
            disabled={!isDirty}
            className="flex-1 btn-brand text-white h-8 text-xs"
          >
            Salvar Alterações
          </Button>
        </footer>
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Esta ação não pode ser desfeita. O preset será removido
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                onClick={() => {
                  if (deletePresetId) {
                    deleteCustomPreset(deletePresetId);
        ...
        <AlertDialog
          open={deleteImagePresetId !== null}
          onOpenChange={(v) => {
            if (!v) setDeleteImagePresetId(null);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Esta ação não pode ser desfeita. O preset de imagem será
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                className="bg-destructive hover:bg-destructive/90"
                onClick={() => {
                  if (deleteImagePresetId) {
                    deleteCustomImagePreset(deleteImagePresetId);
                    setDeleteImagePresetId(null);
                  }
                }}
              >
                Excluir
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Dê um nome para este tema. Ele ficará disponível na lista de
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setPresetName("")}>
                Cancelar
              </AlertDialogCancel>
              <AlertDialogAction
        ...
        <AlertDialog
          open={saveImagePresetOpen}
          onOpenChange={(v) => {
            setSaveImagePresetOpen(v);
            if (!v) setImagePresetName("");
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setImagePresetName("")}>
                Cancelar
              </AlertDialogCancel>
              <AlertDialogAction
