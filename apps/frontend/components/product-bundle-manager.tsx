...
export function ProductBundleManager({
  ...
  function addProduct(p: any) {
    setForm((f) => {
      ...
      return {
        ...
        items: [
          ...f.items,
          { product_id: p.id, variation_id: null, product: p },
        ],
      };
    });
  }
  ...
  function removeProduct(productId: string) {
    setForm((f) => ({
      ...f,
      items: f.items.filter((it) => it.product_id !== productId),
    }));
  }

  async function handleSave() {
    if (!form.name.trim()) {
      toast({ title: "Informe o nome do combo", variant: "destructive" });
      return;
    }
    if (form.items.length < 2) {
      toast({
        title: "Um combo precisa de pelo menos 2 produtos",
        variant: "destructive",
      });
      return;
    }
                  ...
                  <table className="tabela-cartao w-full text-xs min-w-[720px]">
                    <thead>
                      <tr className="border-b border-slate-200/60 dark:border-slate-700/60">
                        ...
                        <th
                          className="py-3.5 px-4 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.04em] text-left"
                          style={{ background: "var(--table-head)" }}
                        >
                          Combo
                        </th>
                        <th
                      ...
                      {filtered.map((b, i) => (
                        ...
                        >
                                    ...
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <button
                                          ...
                                          className="h-[26px] w-[26px] flex items-center justify-center rounded-[8px] bg-white dark:bg-slate-800 border border-[#e8edf5] dark:border-slate-700 text-red-500 dark:text-red-400 shadow-[0_4px_10px_rgba(15,23,42,0.06)] hover:bg-red-600 hover:text-white transition-all"
                                        >
                          ...
                          <td data-rotulo="Categoria" className="py-3 px-4">
                            {b.category ? (
                              <NeonBadge color="blue" className="capitalize">
                                {b.category}
                              </NeonBadge>
                            ) : (
                          ...
                          <td
                            data-rotulo="Produtos"
                            className="py-3 px-4 text-slate-600 dark:text-slate-300"
                          >
                            {(b.items || []).length} produto
                            {(b.items || []).length !== 1 ? "s" : ""}
                          </td>
                            ...
                            <td data-rotulo="Origem" className="py-3 px-4">
                              {b.agency_id ? (
                                <NeonBadge color="violet">
                                  ...
                                  {b.agency?.name || "Agência"}
                                </NeonBadge>
                              ) : (
                                <NeonBadge color="slate">
                          ...
                          <td data-rotulo="Status" className="py-3 px-4">
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-bold w-fit border ${b.is_active ? "border-emerald-500 bg-emerald-200 text-emerald-900 dark:bg-emerald-800/70 dark:text-emerald-100" : "border-slate-400 bg-slate-300 text-slate-800 dark:bg-slate-800 dark:text-slate-300"}`}
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${b.is_active ? "bg-emerald-500" : "bg-slate-400"}`}
                              />
        ...
        <EmbeddedSlideScreen
          ...
          subtitle={
            viewMode
              ...
              : editItem
                ? "Atualize os produtos e dados do combo"
          ...
          footer={
            <div className="flex items-center gap-3">
              ...
              {viewMode ? (
                podeEditar(editItem) && (
                  <Button
                    className="h-10 px-8 btn-brand border-0 shadow-md ml-auto"
          ...
          <div className="flex-1 overflow-y-auto bg-slate-100 dark:bg-slate-900 px-6 py-6 space-y-4">
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                <span className="h-6 w-6 rounded-full bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 text-xs font-bold flex items-center justify-center shrink-0">
                  1
                </span>
              ...
              <div className="px-5 py-5 space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    placeholder="Ex: Combo Presença Digital Completa"
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    placeholder="Descreva o que esse combo entrega..."
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-3">
                  <span className="h-6 w-6 rounded-full bg-violet-100 dark:bg-violet-950/50 text-violet-700 dark:text-violet-400 text-xs font-bold flex items-center justify-center shrink-0">
                    2
                  </span>
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                    Produtos do combo
                ...
                <span className="text-xs font-semibold tabular-nums text-emerald-700 dark:text-emerald-400">
                  Total:{" "}
                  {currency(form.items.reduce((s, it) => s + itemPrice(it), 0))}
                </span>
              </div>
              ...
              <div className="px-5 py-4 space-y-2">
                {form.items.length === 0 ? (
                  ...
                  form.items.map((it, idx) => (
                    <div
                      key={it.product_id}
                      className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-900/30"
                    >
                      <GripVertical className="h-3.5 w-3.5 text-slate-300 shrink-0" />
                      <span className="h-6 w-6 rounded-full bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 text-[10px] font-bold flex items-center justify-center shrink-0">
                        {idx + 1}
                      </span>
                      <div className="min-w-0 flex-1">
                ...
                {!viewMode &&
                  ...
                  form.items.length < 2 && (
                    <p className="text-[11px] text-amber-600 dark:text-amber-400">
                      Adicione pelo menos mais um produto — um combo precisa de
                      2 ou mais.
                    </p>
                  )}
              </div>
