...
// Self-service: o próprio Partner cria/gerencia seus colaboradores via
// /api/partner/users. Não mostra partner_id nem permite escolher outro
// partner — o backend sempre vincula ao partner do usuário logado. Espelha
...
export default function PartnerUsuariosPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data: any = await apiClient.getPartnerUsers(params);
  ...
  // partner_admin nunca é atribuível via este painel self-service (o
  // backend só aceita role="partner_user" aqui) — então
  // role === "partner_admin" identifica com segurança o usuário principal.
  function isPrincipal(u: PartnerUser) {
    return u.role === "partner_admin";
  }
  ...
  async function handleCreate() {
    ...
    setFormError("");
    try {
      await apiClient.createPartnerUser({
        name: form.name,
        email: form.email,
  ...
  function openEdit(u: PartnerUser) {
    setEditUser(u);
  ...
  async function handleEditSave() {
    ...
    try {
      ...
      await apiClient.updatePartnerUser(editUser.id, payload);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <PageHeader
        title="Usuários"
        description="Colaboradores com acesso ao seu perfil de parceiro na plataforma"
        actions={
                ...
                <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3">
                    <p className="flex items-center gap-1.5 font-semibold text-xs text-slate-900 dark:text-slate-100">
                      ...
                      {isPrincipal(u) && (
                        <span
                          title="Usuário principal — administrador do parceiro"
      ...
      <SlidePanel
        ...
        onClose={closeCreate}
        title="Novo usuário"
        subtitle="Colaborador do seu perfil de parceiro"
        widthMode="compact"
        compactWidth={420}
        footer={
        ...
        <div className="p-5 flex-1 overflow-y-auto space-y-4">
          ...
          {editUser && isPrincipal(editUser) && (
            <p className="flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-400/20 rounded-md px-3 py-2">
              ...
              Usuário principal — administrador do parceiro. Bloqueio não pode ser alterado por aqui.
            </p>
          )}
          <div>
