...
const INVOICE_STATUS_CLASSES = {
  ...
  cancelled:
    "bg-slate-100 text-slate-500 border-slate-200 dark:bg-slate-800/50 dark:text-slate-400",
};
...
const WD_STATUS_COLOR = {
  aguardando_analise: "amber",
  pagamento_agendado: "blue",
  pagamento_efetuado: "emerald",
  cancelado: "slate",
  reprovado: "red",
};
...
const WALLET_OWNER_COLOR_TOKEN: Record<string, BadgeColor> = {
  company: "blue",
  agency: "violet",
  nomad: "amber",
  partner: "emerald",
...
const WALLET_STATUS_CLASSES: Record<string, string> = {
  ...
  suspended:
    "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400",
  closed:
    "bg-slate-100 text-slate-400 border-slate-200 dark:bg-slate-700/50 dark:text-slate-500",
};
...
const EXP_STATUS_CLASSES = {
  prevista:
    "bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-800/50 dark:text-slate-400",
  pendente:
    "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400",
...
export default function AdminFinanceiroPage() {
  ...
  const [invVisibleCols, setInvVisibleCols] = useState(
    new Set(DEFAULT_INVOICE_VISIBLE_COLS),
  );
  ...
  const [walletPerPage, setWalletPerPage] = useItemsPerPage(
    "admin-fin-wallet",
    10,
  );
  ...
  const [walletDateRange, setWalletDateRange] = useState<DateRange | undefined>(
    undefined,
  );
  ...
  const [projectionsMode, setProjectionsMode] = useState<"credits" | "debits">(
    "credits",
  );
  ...
  const [squadPerPage, setSquadPerPage] = useItemsPerPage(
    "admin-fin-squad",
    50,
  );
  ...
  const [concilPerPage, setConcilPerPage] = useItemsPerPage(
    "admin-fin-concil",
    25,
  );
  const [concilStats, setConcilStats] = useState<any>(null);
  const [concilDateRange, setConcilDateRange] = useState<DateRange | undefined>(
    undefined,
  );
  ...
  const [activeTab, setActiveTab] = useState<
    "faturas" | "saques" | "despesas" | "carteiras" | "squad" | "conciliacao"
  >("faturas");
  ...
  const [savedFilters, setSavedFilters] = useState<
    { id: string; name: string; status: string; wdStatus: string }[]
  >([]);
  ...
  const loadInvoices = useCallback(async () => {
    ...
    try {
      ...
      const [res, stats, dre] = await Promise.all([
        ...
        dreParams
          ? apiClient.getDRE(dreParams.from, dreParams.to)
    ...
    try {
      ...
      const [res, stats] = await Promise.all([
        ...
        apiClient.getExpenseStats(
          periodFrom ? { from: periodFrom, to: periodTo } : undefined,
        ),
      ]);
  ...
  useEffect(() => {
    if (activeTab === "conciliacao") loadConciliation();
  }, [loadConciliation, activeTab]);
  ...
  const kpi = useMemo(() => {
    if (!billingStats)
      return { paid: 0, pending: 0, overdue: 0, avgTicket: 0, invoiceCount: 0 };
    const map: Record<string, number> = {};
    (billingStats.byStatus || []).forEach((s) => {
      map[s.status] = s.amount;
    });
    return {
      paid: map["paid"] ?? 0,
  ...
  const filteredWithdrawals = useMemo(() => {
    ...
    if (wdStatusFilter !== "all")
      result = result.filter((w) => w.status === wdStatusFilter);
    return result;
  ...
  const filteredPartnerWithdrawals = useMemo(() => {
    ...
    if (pwStatusFilter !== "all")
      result = result.filter((w) => w.status === pwStatusFilter);
    return result;
  }, [partnerWithdrawals, pwSearch, pwStatusFilter]);

  const wdTotalPages = Math.max(
    1,
    Math.ceil(filteredWithdrawals.length / wdPerPage),
  );
  ...
  const pwTotalPages = Math.max(
    1,
    Math.ceil(filteredPartnerWithdrawals.length / pwPerPage),
  );
  const pagedPartnerWithdrawals = filteredPartnerWithdrawals.slice(
    (pwPage - 1) * pwPerPage,
    pwPage * pwPerPage,
  );
  ...
  const {
    sortKey: pwSK,
    sortDir: pwSD,
    handleSort: handlePwSort,
    sortData: sortPartnerWithdrawals,
  } = useSorting();
  const {
    sortKey: expSK,
    sortDir: expSD,
  ...
  const {
    sortKey: waSK,
    sortDir: waSD,
    handleSort: handleWaSort,
    sortData: sortWalletsFn,
  } = useSorting();
  const {
    sortKey: sqSK,
    sortDir: sqSD,
    handleSort: handleSqSort,
    sortData: sortSquad,
  } = useSorting();
  const {
    sortKey: ccSK,
    sortDir: ccSD,
    handleSort: handleCcSort,
    sortData: sortConcil,
  } = useSorting();

  const invTotalPages = Math.max(1, Math.ceil(invoiceTotal / invPerPage));
  const activeFilterCount = [invStatusFilter !== "all", periodActive].filter(
    Boolean,
  ).length;
  ...
  async function handleMarkOverdue(id) {
    ...
    try {
      ...
      toast({ title: "Marcada como em atraso" });
      loadInvoices();
    } catch {
      toast({ title: "Erro", variant: "destructive" });
    } finally {
  ...
  async function approveWithdrawal(id) {
    setActionLoading(id);
    try {
      await apiClient.updateWithdrawal(id, {
        status: "pagamento_efetuado",
        notes: "Aprovado e pago",
      });
  ...
  async function approvePartnerWithdrawal(id) {
    ...
    try {
      await apiClient.updateAdminPartnerWithdrawal(id, { status: "approved" });
      toast({ title: "Saque aprovado" });
      loadPartnerWithdrawals();
    } catch (err: any) {
      toast({
  ...
  async function markPartnerWithdrawalPaid(id) {
    setPwActionLoading(id);
    try {
      await apiClient.updateAdminPartnerWithdrawal(id, { status: "paid" });
  ...
  async function rejectPartnerWithdrawal(id) {
    setPwActionLoading(id);
    try {
      await apiClient.updateAdminPartnerWithdrawal(id, {
        status: "rejected",
        notes: pwRejectNote,
      });
      toast({ title: "Saque reprovado" });
      setPwRejectingId(null);
      setPwRejectNote("");
      loadPartnerWithdrawals();
    } catch (err: any) {
      toast({
  ...
  function openEditExpenseSheet(exp) {
    ...
    setExpForm({
      ...
      type: exp.type || "fixa",
      recurrence: exp.recurrence || "mensal",
  ...
  async function handleSaveExpense() {
    ...
    if (!expForm.amount || isNaN(parseFloat(expForm.amount))) {
      toast({ title: "Informe um valor válido", variant: "destructive" });
      return;
    }
    const payload: Record<string, any> = {
    ...
    if (expForm.due_date)
      payload.due_date = new Date(expForm.due_date).toISOString();
    ...
    if (expForm.competence_month)
      payload.competence_month = expForm.competence_month;
  ...
  const loadWallets = useCallback(async () => {
    ...
    try {
      ...
      const [res, stats] = await Promise.all([
        ...
        apiClient
          .getWalletStats(
            Object.keys(statsParams).length ? statsParams : undefined,
          )
          .catch(() => ({})),
      ]);
      setWallets(res.data || []);
      ...
      apiClient
        .getWalletProjections({ days: walletHorizon })
        .then(setWalletProjections)
        .catch(() => setWalletProjections(null));
    } catch (err) {
      console.error("[Financeiro] wallets:", err);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === "carteiras") loadWallets();
  }, [loadWallets, activeTab]);
    ...
    async (walletId: string) => {
      ...
      try {
        ...
        if (ledgerDateRange?.from)
          params.from = ledgerDateRange.from.toISOString();
        ...
        console.error("[Financeiro] ledger:", err);
      } finally {
  ...
  useEffect(() => {
    if (selectedWallet && ledgerOpen) loadLedger(selectedWallet.id);
  }, [loadLedger, selectedWallet, ledgerOpen]);
  ...
  const loadGlobalLedger = useCallback(async () => {
    ...
    try {
      ...
      if (globalLedgerRefType !== "all")
        params.reference_type = globalLedgerRefType;
      ...
      console.error("[Financeiro] global ledger:", err);
    } finally {
  ...
  useEffect(() => {
    if (globalLedgerOpen) loadGlobalLedger();
  }, [loadGlobalLedger, globalLedgerOpen]);

  const globalLedgerTotalPages = Math.max(1, Math.ceil(globalLedgerTotal / 20));

  function openGlobalLedger(opts: {
    type?: string;
    direction?: string;
    refType?: string;
    title: string;
  ...
  async function handleAdjustment() {
    ...
    try {
      ...
      toast({
        title: `${adjustForm.direction === "credit" ? "Crédito" : "Débito"} lançado com sucesso`,
      });
      setAdjustOpen(false);
      setAdjustForm({
        direction: "credit",
      ...
      loadWallets();
      if (ledgerOpen && selectedWallet?.id === adjustWallet.id)
        loadLedger(adjustWallet.id);
    } catch (err: any) {
      toast({
      ...
      <div className="h-full min-h-0 flex flex-col" ref={pageRef}>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="space-y-0">
            ...
            <div className="mt-[5px] mb-[5px] grid grid-cols-1 gap-3 sm:grid-cols-2 lg:h-[65px] lg:grid-cols-4">
              <div className="relative h-full rounded-xl overflow-hidden shadow-sm border-2 border-emerald-300/70 dark:border-emerald-800/70 bg-gradient-to-br from-emerald-500 to-teal-700 px-3 pt-2 pb-1.5">
                ...
                <p className="text-[10px] text-white/60 mt-0.5">
                  {periodActive ? "No período selecionado" : "Faturas pagas"}
                </p>
              </div>

              <div className="relative h-full rounded-xl overflow-hidden shadow-sm border-2 border-amber-300/70 dark:border-amber-800/70 bg-gradient-to-br from-amber-500 to-orange-600 px-3 pt-2 pb-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-white/70 leading-tight">
                    A Receber
                  </p>
              ...
              <div className="relative h-full rounded-xl overflow-hidden shadow-sm border-2 border-red-300/70 dark:border-red-800/70 bg-gradient-to-br from-red-500 to-rose-700 px-3 pt-2 pb-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-white/70 leading-tight">
                    Em Atraso
                  </p>
                ...
                <p className="text-[10px] text-white/60 mt-0.5">
                  {periodActive ? "No período selecionado" : "Faturas vencidas"}
                </p>
              </div>

              <div className="relative h-full rounded-xl overflow-hidden shadow-sm border-2 border-violet-300/70 dark:border-violet-800/70 bg-gradient-to-br from-violet-500 to-purple-700 px-3 pt-2 pb-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-white/70 leading-tight">
            ...
            <Accordion type="single" collapsible>
              <AccordionItem
                value="periodo"
                className="border rounded-lg bg-blue-50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-900/40"
              >
                      ...
                      <div className="divide-y divide-slate-100 dark:divide-slate-800">
                        ...
                        <div className="flex items-center justify-between px-4 py-2.5">
                          <span className="text-sm text-slate-600 dark:text-slate-300">
                            <span className="text-green-600 dark:text-green-400 font-bold mr-1">
                              (+)
                            </span>
                            Receita Bruta
                          </span>
                        ...
                        <div className="flex items-center justify-between px-4 py-2.5">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-0.5 pl-4">
                              Pagamentos de nômades efetuados
                            </p>
                          </div>
                          <span className="text-sm font-semibold text-red-600 dark:text-red-400 tabular-nums">
                            {fmt(dreData.custosDiretos)}
                          </span>
                        </div>
                        {/* Lucro Bruto */}
                        <div
                          className={`flex items-center justify-between px-4 py-3 ${dreData.lucroBruto >= 0 ? "bg-green-50 dark:bg-green-950/20" : "bg-red-50 dark:bg-red-950/20"}`}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span
                              className={`text-xs font-bold px-2 py-0.5 rounded-full ${dreData.margemBruta >= 0 ? "bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300" : "bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300"}`}
                            >
                              {dreData.margemBruta}%
                            </span>
                          </div>
                          <span
                            className={`text-sm font-bold tabular-nums ${dreData.lucroBruto >= 0 ? "text-green-700 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                          >
                        ...
                        <div className="flex items-center justify-between px-4 py-2.5">
                          ...
                          <span className="text-sm font-semibold text-red-600 dark:text-red-400 tabular-nums">
                            {fmt(dreData.despesasOperacionais)}
                          </span>
                        </div>
                        {/* Breakdown por categoria */}
                        {(dreData.despesasPorCategoria || []).map(
                          (cat: any) => (
                            <div
                              key={cat.category}
                              className="flex items-center justify-between px-4 py-1.5 bg-slate-50/50 dark:bg-slate-800/30"
                            >
                        ...
                        {/* Lucro Operacional */}
                        <div
                          className={`flex items-center justify-between px-4 py-3 ${dreData.lucroOperacional >= 0 ? "bg-blue-50 dark:bg-blue-950/20" : "bg-red-50 dark:bg-red-950/20"}`}
                        >
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-bold text-slate-800 dark:text-slate-100">
                              Lucro Operacional
                            ...
                            <span
                              className={`text-xs font-bold px-2 py-0.5 rounded-full ${dreData.margemOperacional >= 0 ? "bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300" : "bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300"}`}
                            >
                              {dreData.margemOperacional}%
                            </span>
                          </div>
                          <span
                            className={`text-sm font-bold tabular-nums ${dreData.lucroOperacional >= 0 ? "text-blue-700 dark:text-blue-400" : "text-red-600 dark:text-red-400"}`}
                          >
                            {fmt(dreData.lucroOperacional)}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  {periodActive && billingStats && (
                    <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {(billingStats.byStatus || []).map((s) => (
                        <div
                          key={s.status}
                          className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 px-3 py-2"
                        >
                  ...
                  <>
                    ...
                    <div className="flex-1 relative min-w-0 max-w-sm">
                      ...
                      <Input
                        placeholder={
                          activeTab === "faturas"
                            ? "Buscar fatura, empresa..."
                            : activeTab === "saques"
                              ? "Buscar parceiro, PIX..."
                              : activeTab === "despesas"
                                ? "Buscar despesa, categoria..."
                                : activeTab === "squad"
                                  ? "Buscar empresa Squad..."
                                  : activeTab === "conciliacao"
                                    ? "Buscar por descrição, titular..."
                                    : "Buscar carteira, titular, e-mail..."
                        }
                        value={
                          activeTab === "faturas"
                            ? invSearch
                        ...
                        onChange={(e) => {
                          if (activeTab === "faturas")
                            ...
                            setConcilSearch(e.target.value);
                            setConcilPage(1);
                          } else {
                          ...
                          activeTab === "faturas"
                            ...
                            : activeTab === "saques"
                              ...
                              : activeTab === "despesas"
                                ? expPerPage.toString()
                                : activeTab === "squad"
                                  ? squadPerPage.toString()
                                  : activeTab === "conciliacao"
                                    ? concilPerPage.toString()
                                    : walletPerPage.toString()
                        }
                        onValueChange={(v) => {
                          if (activeTab === "faturas") {
                            setInvPerPage(Number(v));
                            setInvPage(1);
                          } else if (activeTab === "saques") {
                            setWdPerPage(Number(v));
                            setWdPage(1);
                          } else if (activeTab === "despesas") {
                            setExpPerPage(Number(v));
                            setExpPage(1);
                          } else if (activeTab === "squad") {
                            setSquadPerPage(Number(v));
                          } else if (activeTab === "conciliacao") {
                            setConcilPerPage(Number(v));
                            setConcilPage(1);
                          } else {
                            setWalletPerPage(Number(v));
                            setWalletPage(1);
                          }
                        }}
                      />
                      <span className="text-xs text-slate-400 whitespace-nowrap">
                        {activeTab === "faturas" ? (
                          ...
                          <>
                            ...
                            saque{filteredWithdrawals.length !== 1 ? "s" : ""}
                          </>
                        ) : activeTab === "despesas" ? (
                          <>
                            de{" "}
                          ...
                          activeTab === "faturas"
                            ...
                            : activeTab === "saques"
                              ...
                              : activeTab === "despesas"
                                ...
                                : walletActiveFilterCount;
                        return (
                          <IconToolbarButton
                            icon={Filter}
                            tooltip={
                              cnt > 0 ? `Filtros (${cnt} ativos)` : "Filtros"
                            }
                    ...
                    >
                      ...
                      <Cog
                        className={cn(
                          ...
                          colConfigOpen
                            ? "text-white"
                            : "text-[#7d1b6a] dark:text-[#c07ab0] group-hover:text-white",
                        )}
                      />
                    </button>
                  </PopoverTrigger>
                ...
                <Card className="overflow-hidden">
                        ...
                        <tr>
                          <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">
                            <SortableHeader
                              ...
                              type="text"
                              sortKey={invSK ? String(invSK) : null}
                              sortDir={invSD}
                          ...
                          <th
                            ...
                            style={{
                              ...
                              background: "var(--table-head)",
                              boxShadow:
                                "-1px 0 0 rgba(100,116,139,0.18), 0 1px 0 rgba(148,163,184,0.3)",
                            }}
                          >
                            Ações
                          </th>
                            ...
                            return (
                              ...
                              >
                                ...
                                >
                                  ...
                                  {inv.description && (
                                    <p className="text-[11px] text-slate-400 mt-0.5 max-w-[180px] truncate">
                                      {inv.description}
                                    </p>
                                  )}
                                </td>
                                ...
                                {invVisibleCols.has("valor") && (
                                  <td
                                    data-rotulo="Valor"
                                    className="px-4 py-3 text-right font-bold tabular-nums text-slate-800 dark:text-slate-200"
                                  >
                                    {fmt(inv.amount)}
                                  </td>
                                )}
                                {invVisibleCols.has("vencimento") && (
                                  ...
                                  >
                                    <div className="flex items-center gap-1">
                                      ...
                                      <span
                                        className={`text-xs ${isOverdue ? "text-red-600 font-semibold" : "text-slate-500 dark:text-slate-400"}`}
                                      >
                  ...
                  <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                    <p className="text-xs text-slate-400 shrink-0">
                      {invoiceTotal} fatura{invoiceTotal !== 1 ? "s" : ""}
                    </p>
                    {hasHorizontalOverflow && (
                      <div
                        ref={bottomScrollRef}
            ...
            na mesma tabela pra não confundir qual regra de saldo se aplica. */}
              {activeTab === "saques" && (
                <div className="flex items-center gap-2">
                  ...
                  >
                    Nômades
                    {wdPending > 0 && (
                      <span className="opacity-70">({wdPending})</span>
                    )}
                  </button>
                  <button
                  ...
                  >
                    Partners
                    {pwPending > 0 && (
                      <span className="opacity-70">({pwPending})</span>
                    )}
                  </button>
                </div>
              )}
              ...
              {activeTab === "saques" && wdSubTab === "nomades" && (
                <Card className="overflow-hidden">
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            setWdStatusFilter(
                              wdStatusFilter === status ? "all" : status,
                            )
                          }
                          className={cn(
                        ...
                        >
                          ...
                          {amount > 0 && (
                            <span className="opacity-60">· {fmt(amount)}</span>
                          )}
                        </button>
                      );
                    })}
                          ...
                          sortWithdrawals(pagedWithdrawals).map((w, idx) => (
                            ...
                            >
                              ...
                              <td data-rotulo="Status" className="px-4 py-3">
                                <NeonBadge
                                  color={WD_STATUS_COLOR[w.status] || "slate"}
                                >
                              ...
                              <td
                                ...
                                style={{
                                  ...
                                  right: 0,
                                  zIndex: 1,
                                  boxShadow: "-1px 0 0 rgba(100,116,139,0.18)",
                                }}
                              >
                                  ...
                                  rejectingId === w.id ? (
                                    <div className="flex gap-1.5 items-center">
                                      ...
                                      <button
                                        ...
                                        className="h-7 px-2 rounded border border-slate-200 dark:border-slate-600 text-slate-500 text-xs"
                                      >
                                        X
                                      </button>
                                    </div>
                  ...
                  <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                    <p className="text-xs text-slate-400 shrink-0">
                      {filteredWithdrawals.length} solicitação
                      {filteredWithdrawals.length !== 1 ? "ões" : ""}
                    </p>
                    {hasHorizontalOverflow && (
                      <div
                        ref={bottomScrollRef}
                ...
                <Card className="overflow-hidden">
                  ...
                  <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                    ...
                    {[
                      ...
                      const count = partnerWithdrawals.filter(
                        (w) => w.status === status,
                      ).length;
                      if (count === 0) return null;
                      return (
                        <button
                          ...
                          onClick={() => {
                            setPwStatusFilter(
                              pwStatusFilter === status ? "all" : status,
                            );
                            setPwPage(1);
                          }}
                          className={cn(
                    ...
                    <div className="ml-auto flex items-center gap-2">
                      <ItemsPerPageSelect
                        value={pwPerPage.toString()}
                        onValueChange={(v) => {
                          setPwPerPage(Number(v));
                          setPwPage(1);
                        }}
                      />
                      <span className="text-xs text-slate-400 whitespace-nowrap">
                        de{" "}
                        <span className="font-semibold text-slate-600 dark:text-slate-300">
                          {filteredPartnerWithdrawals.length}
                        </span>{" "}
                        saque
                        {filteredPartnerWithdrawals.length !== 1 ? "s" : ""}
                      </span>
                    </div>
                  </div>
                    ...
                    <table className="tabela-cartao w-full text-sm min-w-[700px]">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {pwLoading ? (
                          ...
                          sortPartnerWithdrawals(pagedPartnerWithdrawals).map(
                            (w, idx) => (
                              <tr
                                key={w.id}
                                className={
                              ...
                              >
                                ...
                                <td
                                  data-rotulo="Processado em"
                                  className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400"
                                >
                                  {w.paidAt ? fmtDateTime(w.paidAt) : "—"}
                                </td>
                                <td data-rotulo="Status" className="px-4 py-3">
                                  <NeonBadge
                                    color={
                                      PARTNER_WD_STATUS_COLOR[w.status] ||
                                      "slate"
                                    }
                                  >
                                    {PARTNER_WD_STATUS_LABELS[w.status] ||
                                      w.status}
                                  </NeonBadge>
                                    ...
                                    pwRejectingId === w.id ? (
                                      <div className="flex gap-1.5 items-center">
                                        <input
                                          ...
                                          value={pwRejectNote}
                                          onChange={(e) =>
                                            setPwRejectNote(e.target.value)
                                          }
                                        />
                                        <button
                                          onClick={() =>
                                            rejectPartnerWithdrawal(w.id)
                                          }
                                          className="h-7 px-2 rounded bg-red-600 text-white text-xs font-semibold hover:bg-red-700"
                                        >
                                          OK
                                      ...
                                      <div className="flex gap-1.5">
                                        {w.status === "pending" && (
                                          <button
                                            disabled={pwActionLoading === w.id}
                                            onClick={() =>
                                              approvePartnerWithdrawal(w.id)
                                            }
                                        ...
                                        <button
                                          disabled={pwActionLoading === w.id}
                                          onClick={() =>
                                            markPartnerWithdrawalPaid(w.id)
                                          }
                                          className="h-7 px-2.5 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-semibold flex items-center gap-1 transition-colors disabled:opacity-40"
                                        >
                  ...
                  <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                    <p className="text-xs text-slate-400 shrink-0">
                      {filteredPartnerWithdrawals.length} solicitação
                      {filteredPartnerWithdrawals.length !== 1 ? "ões" : ""}
                    </p>
                    <MiniPagination
                      curPage={pwPage}
                      totalPages={pwTotalPages}
                      onChange={setPwPage}
                    />
                  </div>
                </Card>
                    ...
                    {[
                      {
                        ...
                        sub: `${expStats?.count ?? 0} despesa${(expStats?.count ?? 0) !== 1 ? "s" : ""}`,
                        from: "from-slate-600",
                        to: "to-slate-800",
                      },
                      {
                        label: "Pagas",
                        value: expStats?.paid ?? 0,
                        sub: "Quitadas",
                      ...
                      {
                        label: "Variáveis",
                        value: expStats?.variable ?? 0,
                        sub: "Despesas variáveis",
                      ...
                      <div
                        key={label}
                        className={`relative rounded-xl overflow-hidden shadow-sm bg-gradient-to-br ${from} ${to} px-3 pt-2 pb-1.5`}
                      >
                        <p className="text-xs font-medium text-white/70 leading-tight mb-1">
                          {label}
                        </p>
                  ...
                  <Card className="overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 flex-wrap px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                      {(
                        ...
                        const row = expStats?.byStatus?.find(
                          (s) => s.status === status,
                        );
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              setExpStatusFilter(
                                expStatusFilter === status ? "all" : status,
                              );
                              setExpPage(1);
                            }}
                            className={cn(
                          ...
                          <tr>
                            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                              <SortableHeader
                                ...
                                field="name"
                                type="text"
                                sortKey={expSK ? String(expSK) : null}
                                sortDir={expSD}
                            ...
                            sortExpenses(expenses).map((exp, idx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td
                                    data-rotulo="Competência"
                                    className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400"
                                  >
                                    {exp.competence_month || "—"}
                                  </td>
                                  <td
                                    data-rotulo="Vencimento"
                                    className="px-4 py-3"
                                  ...
                                  >
                                    <NeonBadge
                                      color={
                                        EXP_STATUS_COLOR[exp.status] || "slate"
                                      }
                                    >
                    ...
                    <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                      <p className="text-xs text-slate-400 shrink-0">
                        {expenseTotal} despesa{expenseTotal !== 1 ? "s" : ""}
                      </p>
                      {hasHorizontalOverflow && (
                        <div
                          ref={bottomScrollRef}
              ...
              {activeTab === "carteiras" && (
                <>
                    ...
                    {[
                      {
                        ...
                        sub: `${walletStats?.walletCount ?? 0} carteiras`,
                        from: "from-blue-600",
                        to: "to-indigo-800",
                        icon: <Wallet className="h-4 w-4 text-white/60" />,
                      },
                      {
                        label: "Saldo Bloqueado",
                        value: walletStats?.blockedBalance ?? 0,
                        sub: "Em análise/restrição",
                        from: "from-amber-500",
                        to: "to-orange-700",
                        icon: <Info className="h-4 w-4 text-white/60" />,
                      },
                      {
                        label: "Carteiras Ativas",
                        value: walletStats?.activeCount ?? 0,
                        sub: `${walletStats?.suspendedCount ?? 0} suspensas`,
                        from: "from-violet-500",
                        to: "to-purple-700",
                      ...
                      },
                      {
                        label: "Saldo Líquido",
                        value:
                          (walletStats?.credits ?? 0) -
                          (walletStats?.debits ?? 0),
                        sub: walletDateRange
                          ? "Créditos − Débitos no período"
                      ...
                      ({
                        ...
                        <div
                          key={label}
                          className={`relative rounded-xl overflow-hidden shadow-sm bg-gradient-to-br ${from} ${to} px-4 pt-3 pb-2.5`}
                        >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">
                        Créditos{" "}
                        {walletDateRange ? "no período" : "(total geral)"}
                      </span>
                      ...
                      {[
                        {
                          ...
                          value: walletStats?.credits ?? 0,
                          count: walletStats?.creditCount ?? 0,
                          color:
                            "border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/20",
                          badge:
                          ...
                          onClick: () =>
                            openGlobalLedger({
                              direction: "credit",
                              title: "Créditos — todos",
                            }),
                          tooltip: "Total de entradas nas carteiras no período",
                        },
                        {
                          label: "Bônus",
                          value: walletStats?.bonus ?? 0,
                          count: walletStats?.bonusCount ?? 0,
                          color:
                            "border-pink-400 hover:bg-pink-50 dark:hover:bg-pink-950/20",
                          badge:
                          ...
                          tooltip: "Bônus manuais, promocionais e de ativação",
                        },
                        {
                          label: "Crédito adicional",
                          value: walletStats?.additionalCredit ?? 0,
                          count: walletStats?.additionalCreditCount ?? 0,
                          color:
                            "border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/20",
                          badge:
                          ...
                          tooltip:
                            "Ajustes positivos, recargas e créditos manuais",
                        },
                        {
                          label: "Plano",
                          value: walletStats?.planCredits ?? 0,
                          count: walletStats?.planCreditCount ?? 0,
                          color:
                            "border-violet-400 hover:bg-violet-50 dark:hover:bg-violet-950/20",
                          badge:
                          ...
                          onClick: () =>
                            openGlobalLedger({
                              refType: "invoice",
                              direction: "credit",
                              title: "Entradas de planos",
                            }),
                          tooltip:
                            "Pagamentos de assinatura e renovação de planos",
                        },
                        {
                          label: "Projetos recorrentes",
                          value: walletStats?.recurringCredits ?? 0,
                          count: walletStats?.recurringCreditCount ?? 0,
                          color:
                            "border-teal-400 hover:bg-teal-50 dark:hover:bg-teal-950/20",
                          badge:
                          ...
                          tooltip:
                            "Pagamentos de projetos com ciclo mensal/recorrente",
                        },
                        {
                          label: "Comissões",
                          value: walletStats?.commissions ?? 0,
                          count: walletStats?.commissionCount ?? 0,
                          color:
                            "border-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/20",
                          badge:
                          ...
                          tooltip:
                            "Comissões de agências, partners e indicações",
                        },
                      ].map(
                        ({
                          label,
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              {icon}
                              <span
                                className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${badge}`}
                              >
                                crédito
                              </span>
                            </div>
                            <p className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 leading-tight mb-1">
                              {label}
                            </p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-100 tabular-nums leading-none">
                              {fmt(value)}
                            </p>
                            <p className="text-[10px] text-slate-400 mt-1">
                              {count} transação{count !== 1 ? "ões" : ""}
                            </p>
                            <p className="text-[10px] text-blue-500 dark:text-blue-400 mt-1 opacity-0 group-hover:opacity-100 transition-opacity font-medium">
                              Ver detalhes →
                            </p>
                          </button>
                        ),
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        {
                          label: "Saídas totais",
                          value: walletStats?.debits ?? 0,
                          count: walletStats?.debitCount ?? 0,
                          color:
                            "border-red-400 hover:bg-red-50 dark:hover:bg-red-950/20",
                          badge:
                          ...
                          tooltip:
                            "Total de saídas das carteiras, incluindo saques",
                        },
                        {
                          label: "Saques",
                          value: walletStats?.withdrawals ?? 0,
                          count: walletStats?.withdrawalCount ?? 0,
                          color:
                            "border-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/20",
                          badge:
                            "bg-rose-100 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400",
                          icon: (
                            <ArrowDownRight className="h-4 w-4 text-rose-500 dark:text-rose-400" />
                          ),
                          onClick: () =>
                            openGlobalLedger({
                              type: "withdrawal",
                          ...
                          tooltip: "Saques aprovados e concluídos no período",
                        },
                      ].map(
                        ({
                          label,
                  ...
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/30 p-4">
                    <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded-full font-semibold">
                          próximos {walletHorizon} dias
                        </span>
                      </div>
                      {/* Horizon selector */}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {/* Créditos futuros */}
                      <button
                        onClick={() => openProjectionsSheet("credits")}
                      ...
                      >
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="ml-auto text-[9px] font-bold bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full">
                            previsão
                          </span>
                        </div>
                        <p className="text-2xl font-bold text-emerald-700 dark:text-emerald-400 tabular-nums">
                          {fmt(walletProjections?.futureCredits ?? 0)}
                        </p>
                        <p className="text-[10px] text-slate-400 mt-1">
                        ...
                        <p className="text-[10px] text-blue-500 dark:text-blue-400 mt-2 opacity-0 group-hover:opacity-100 transition-opacity font-medium">
                          Ver faturas previstas →
                        </p>
                      </button>

                      {/* Débitos futuros */}
                      <button
                        onClick={() => openProjectionsSheet("debits")}
                      ...
                      >
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="ml-auto text-[9px] font-bold bg-orange-100 dark:bg-orange-950/40 text-orange-700 dark:text-orange-400 px-2 py-0.5 rounded-full">
                            previsão
                          </span>
                        </div>
                        <p className="text-2xl font-bold text-orange-700 dark:text-orange-400 tabular-nums">
                          {fmt(walletProjections?.futureDebits ?? 0)}
                        </p>
                        <p className="text-[10px] text-slate-400 mt-1">
                          {walletProjections?.recurringProjects?.length ?? 0}{" "}
                          projeto
                          {(walletProjections?.recurringProjects?.length ??
                            0) !== 1
                            ? "s"
                            : ""}{" "}
                          recorrente
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {walletStats.byType.map((bt) => (
                        ...
                        >
                          <span className="font-semibold">
                            {bt.count}{" "}
                            {WALLET_OWNER_LABELS[bt.owner_type] ||
                              bt.owner_type}
                          </span>
                          <span className="opacity-60">
                            · {fmt(bt.balance)}
                  ...
                  <Card className="overflow-hidden">
                              ...
                              >
                                <div className="flex flex-col items-center gap-2">
                                  ...
                                  <p className="text-[11px]">
                                    Carteiras são criadas automaticamente ao
                                    cadastrar contas
                                  </p>
                                </div>
                              </td>
                            ...
                            sortWalletsFn(wallets).map((w, idx) => (
                              ...
                              >
                                ...
                                <td data-rotulo="Perfil" className="px-4 py-3">
                                  <NeonBadge
                                    color={
                                      WALLET_OWNER_COLOR_TOKEN[w.owner_type] ||
                                      "slate"
                                    }
                                  >
                                    {WALLET_OWNER_LABELS[w.owner_type] ||
                                      w.owner_type}
                                  </NeonBadge>
                                </td>
                                <td
                                ...
                                >
                                  <p
                                    className={`font-bold tabular-nums text-sm ${w.balance > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}
                                  >
                                    {fmt(w.balance)}
                                  </p>
                                </td>
                                <td
                                  data-rotulo="Bloqueado"
                                  className="px-4 py-3 text-right"
                                >
                                  {w.blocked_balance > 0 ? (
                                    <p className="text-amber-600 font-semibold text-xs tabular-nums">
                                      {fmt(w.blocked_balance)}
                                    </p>
                                  ) : (
                                ...
                                <td
                                  data-rotulo="Atualizada"
                                  className="px-4 py-3 text-xs text-slate-400 dark:text-slate-500"
                                >
                    ...
                    <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                      <p className="text-xs text-slate-400 shrink-0">
                        {walletTotal} carteira{walletTotal !== 1 ? "s" : ""}
                      </p>
                      {hasHorizontalOverflow && (
                        <div
                          ref={bottomScrollRef}
              ...
              {activeTab === "squad" && (
                <>
                    ...
                    {[
                      {
                        ...
                        value: squadStats?.activeSquad ?? 0,
                        fmt: (v) => v,
                      ...
                      {
                        label: "Limite total",
                        value: squadStats?.totalCreditLimit ?? 0,
                        fmt,
                      ...
                      },
                      {
                        ...
                        value: squadStats?.totalCreditUsed ?? 0,
                        fmt,
                      ...
                      {
                        label: "Disponível",
                        value: squadStats?.totalCreditAvailable ?? 0,
                        fmt,
                      ...
                      <div
                        key={label}
                        className={`rounded-xl border-2 ${color} bg-white dark:bg-slate-900 p-4 shadow-sm`}
                      >
                        <div className="flex items-center justify-between mb-2">
                          {icon}
                        </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      {
                        label: "Mínimo mensal total",
                        value: squadStats?.totalMonthlyMinimum ?? 0,
                        fmt,
                      ...
                      },
                      {
                        ...
                        value: squadStats?.openInvoices ?? 0,
                        fmt: (v) => v,
                      ...
                      },
                      {
                        ...
                        value: squadStats?.overdueInvoices ?? 0,
                        fmt: (v) => v,
                      ...
                      <div
                        key={label}
                        className={`rounded-xl border ${color} bg-white dark:bg-slate-900 p-3 shadow-sm`}
                      >
                        <div className="flex items-center justify-between mb-1">
                          {icon}
                        </div>
                        <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide leading-tight mb-1">
                          {label}
                        </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setSquadAddOpen(true);
                      }}
                      className="h-8 gap-1.5 bg-gradient-to-r from-blue-500 to-purple-600 text-white text-xs"
                    >
                            ...
                            <tr>
                              ...
                              >
                                <div className="flex flex-col items-center gap-2">
                                  ...
                                  <p className="text-[11px]">
                                    Clique em "Adicionar empresa" para incluir a
                                    primeira
                                  </p>
                                </div>
                              </td>
                            ...
                            sortSquad(
                              squadList.filter(
                                (s) =>
                                  ...
                                  s.company?.name
                                    ?.toLowerCase()
                                    .includes(squadSearch.toLowerCase()),
                              ),
                            )
                              ...
                              .map((sq, idx) => {
                                ...
                                const available =
                                  (sq.credit_limit ?? 0) + walletBalance;
                                const statusMap = {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td
                                      data-rotulo="Mínimo/mês"
                                      className="px-4 py-3 text-right"
                                    >
                                      <span className="font-semibold tabular-nums text-xs text-slate-600 dark:text-slate-300">
                                        {fmt(sq.monthly_minimum)}
                                      </span>
                                    </td>
                                    <td
                                      data-rotulo="Saldo carteira"
                                      className="px-4 py-3 text-right"
                                    >
                                      <span
                                        className={`font-bold tabular-nums text-xs ${walletBalance >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
                                      >
                                        {fmt(walletBalance)}
                                      </span>
                                    </td>
                                    <td
                                      data-rotulo="Disponível"
                                      className="px-4 py-3 text-right"
                                    >
                                      <span
                                        className={`font-bold tabular-nums text-xs ${available > 0 ? "text-blue-600 dark:text-blue-400" : "text-red-600 dark:text-red-400"}`}
                                      >
                                        {fmt(available)}
                                      </span>
                                    </td>
                                    <td
                                      data-rotulo="Fechamento"
                                      ...
                                      <div className="flex items-center justify-end gap-1">
                                        <button
                                          onClick={async () => {
                                            ...
                                            try {
                                              const cycle =
                                                await apiClient.getSquadCurrentCycle(
                                                  sq.id,
                                                );
                                              setSquadCurrentCycle(cycle);
                                            } catch {
                                              setSquadCurrentCycle(null);
                                            } finally {
                                              setSquadCycleLoading(false);
                                            }
                                          }}
                                          className="h-7 px-2 rounded-md flex items-center gap-1 text-[11px] font-medium text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors"
                                        >
                ...
                <>
                  ...
                  <div className="flex items-start gap-3 rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30 px-4 py-3">
                    ...
                    <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                      Este relatório considera apenas movimentações com entrada
                      ou saída real de dinheiro, para conferência com extratos
                      bancários. Bônus, créditos promocionais e movimentações
                      internas de projetos <strong>não entram</strong> na
                      conciliação.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {/* Perfil */}
                    <Select
                      value={concilOwnerType}
                      onValueChange={(v) => {
                        setConcilOwnerType(v);
                        setConcilPage(1);
                      }}
                    >
                    ...
                    {[
                      {
                        ...
                        sub: `${concilStats?.bankInCount ?? 0} transações`,
                      },
                      {
                        label: "Saídas reais",
                        value: concilStats?.bankOut ?? 0,
                        fmt,
                        ...
                        sub: `${concilStats?.bankOutCount ?? 0} transações`,
                      },
                      {
                        label: "Saldo líquido real",
                        value: concilStats?.netReal ?? 0,
                        fmt,
                        color: `${(concilStats?.netReal ?? 0) >= 0 ? "border-blue-400" : "border-orange-400"}`,
                        badge: `${(concilStats?.netReal ?? 0) >= 0 ? "bg-blue-100 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400" : "bg-orange-100 text-orange-700"}`,
                      ...
                      },
                      {
                        ...
                        value:
                          (concilStats?.bankInCount ?? 0) +
                          (concilStats?.bankOutCount ?? 0),
                        fmt: (v) => v,
                        color: "border-slate-300 dark:border-slate-600",
                        badge:
                          "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400",
                        icon: <FileText className="h-4 w-4 text-slate-500" />,
                        sub: "conciliáveis no período",
                      },
                    ].map(
                      ({
                        label,
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      {
                        label: "Saques pagos",
                        value: concilStats?.withdrawals ?? 0,
                        fmt,
                        sub: `${concilStats?.withdrawalCount ?? 0} saques`,
                      ...
                      },
                      {
                        ...
                        value: concilStats?.realCredits ?? 0,
                        fmt,
                        sub: `${concilStats?.realCreditCount ?? 0} entradas`,
                      ...
                      {
                        label: "Carteiras com movimento",
                        value: concilStats?.walletsWithMovement ?? 0,
                        fmt: (v) => v,
                      ...
                      {
                        label: "Período",
                        value: concilDateRange?.from
                          ? `${fmtDate(concilDateRange.from)} → ${fmtDate(concilDateRange.to)}`
                          : "Sem filtro",
                        fmt: (v) => v,
                        sub: "intervalo selecionado",
                        color: "border-slate-200",
                        icon: (
                  ...
                  <Card className="overflow-hidden">
                          ...
                          {concilLoading ? (
                            ...
                            <tr>
                              ...
                              >
                                <div className="flex flex-col items-center gap-2">
                                  ...
                                  <p>
                                    Nenhuma movimentação bancária real
                                    encontrada
                                  </p>
                                  <p className="text-[11px]">
                                    Ajuste o período ou os filtros para ver as
                                    transações
                                  </p>
                                </div>
                              </td>
                            ...
                            sortConcil(concilData).map((entry, idx) => {
                              ...
                              const originLabels = {
                                ...
                                plan: "Plano",
                                recharge: "Recarga",
                                additional_credit: "Créd. Adicional",
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    <NeonBadge
                                      color={
                                        ownerColors[entry.wallet?.owner_type] ||
                                        "slate"
                                      }
                                    >
                                      {ownerLabels[entry.wallet?.owner_type] ||
                                        entry.wallet?.owner_type ||
                                        "—"}
                                    </NeonBadge>
                                  </td>
                                  <td data-rotulo="Tipo" className="px-4 py-3">
                                    <NeonBadge
                                      color={isBankIn ? "emerald" : "red"}
                                  ...
                                  >
                                    <span className="text-[11px] text-slate-600 dark:text-slate-400 font-medium">
                                      {originLabels[entry.type] || entry.type}
                                    </span>
                                  </td>
                                  <td
                                  ...
                                  >
                                    <span
                                      className={`font-bold tabular-nums text-sm ${isBankIn ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
                                    >
                                      {isBankIn ? "+" : "−"}
                                      {fmt(entry.amount)}
                                    </span>
                                  ...
                                  >
                                    {entry.reference_type ? (
                                      <div>
                                        ...
                                        <p
                                          className="text-[10px] text-slate-400 font-mono truncate max-w-[100px]"
                    ...
                    {concilTotal > 0 && (
                      <div className="px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                        <p className="text-xs text-slate-400 shrink-0">
                          {concilTotal} transaç
                          {concilTotal !== 1 ? "ões" : "ão"} conciliável
                          {concilTotal !== 1 ? "is" : ""}
                        </p>
                        {hasHorizontalOverflow && (
                          <div
                        ...
                        <MiniPagination
                          ...
                          totalPages={Math.max(
                            1,
                            Math.ceil(concilTotal / concilPerPage),
                          )}
                          onChange={setConcilPage}
                        />
                      </div>
                    )}
                  </Card>
              ...
              >
                <div className="bg-white dark:bg-slate-900 w-full h-full flex flex-col overflow-hidden">
                  ...
                  >
                    <div className="min-w-0 flex-1 text-sm font-bold text-white truncate">
                      ...
                      <p className="text-[11px] font-normal text-white/60 mt-0.5 truncate">
                        Plano pós-pago com limite de crédito
                      </p>
                    </div>
                    <button
                      onClick={() => {
                        setSquadAddOpen(false);
                        setSquadEditTarget(null);
                      }}
                    ...
                    <div className="max-w-3xl mx-auto space-y-4">
                      {!squadEditTarget && (
                        <div>
                          ...
                          <Select
                            value={squadForm.company_id}
                            onValueChange={(v) =>
                              setSquadForm((f) => ({ ...f, company_id: v }))
                            }
                          >
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            step="100"
                            placeholder="Ex: 2000"
                            value={squadForm.monthly_minimum}
                            onChange={(e) =>
                              setSquadForm((f) => ({
                                ...f,
                                monthly_minimum: e.target.value,
                              }))
                            }
                            className="h-9 text-sm"
                          />
                  ...
                  <div className="px-5 py-3 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
                    ...
                    <Button
                      size="sm"
                      disabled={squadFormLoading}
                      onClick={async () => {
                        if (!squadForm.credit_limit) {
                          toast({
                            title: "Informe o limite de crédito",
                            variant: "destructive",
                          });
                          return;
                        }
                        if (!squadEditTarget && !squadForm.company_id) {
                          toast({
                        ...
                        try {
                          const payload = {
                            ...
                            monthly_minimum: parseFloat(
                              squadForm.monthly_minimum || "0",
                            ),
                            billing_day: parseInt(squadForm.billing_day || "1"),
                            payment_terms: parseInt(
                              squadForm.payment_terms || "30",
                            ),
                          ...
                          if (squadEditTarget) {
                            ...
                            await apiClient.createSquad({
                              ...payload,
                              company_id: squadForm.company_id,
                            });
                            toast({ title: "Empresa adicionada ao Squad" });
                          }
                          setSquadAddOpen(false);
                          setSquadEditTarget(null);
                          loadSquad();
            ...
            <Sheet
              open={!!squadDetailTarget}
              onOpenChange={(o) => {
                if (!o) {
                  setSquadDetailTarget(null);
                  setSquadCurrentCycle(null);
                }
              }}
            >
              <SheetContent className="w-full max-w-xl overflow-y-auto">
                <SheetHeader>
                  <SheetTitle className="text-base">
                    {squadDetailTarget?.company?.name} — Ciclo atual
                  </SheetTitle>
                </SheetHeader>
                {squadCycleLoading ? (
                  ...
                  <div className="mt-4 space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        {
                          label: "Início",
                          value: fmtDate(squadCurrentCycle.cycle?.start_date),
                        },
                        {
                          label: "Fechamento previsto",
                          value: fmtDate(squadCurrentCycle.cycle?.end_date),
                        },
                        {
                          label: "Mínimo mensal",
                          value: fmt(squadDetailTarget?.monthly_minimum),
                        },
                        {
                          label: "Consumo no ciclo",
                          value: fmt(squadCurrentCycle.totalConsumed),
                        },
                        {
                          label: "Limite de crédito",
                          value: fmt(squadDetailTarget?.credit_limit),
                        },
                        {
                          label: "Saldo carteira",
                          value: fmt(squadCurrentCycle.walletBalance),
                        },
                      ].map(({ label, value }) => (
                        <div
                          key={label}
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {squadCurrentCycle.entries.map((e) => (
                            <div
                              key={e.id}
                              className="flex items-center justify-between py-2 px-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 text-xs"
                            >
                              <div>
                              ...
                              <span
                                className={`font-bold tabular-nums ${e.direction === "credit" ? "text-emerald-600" : "text-red-600"}`}
                              >
                                {e.direction === "credit" ? "+" : "−"}
                                {fmt(e.amount)}
                              </span>
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            await apiClient.closeSquadCycle(
                              squadDetailTarget.id,
                            );
                ...
                footer={
                  <div className="flex items-center justify-between gap-2 w-full">
                    <button
                      ...
                      className="text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:underline transition-colors"
                    >
                      Limpar filtros
                    </button>
                              ...
                              if (e.key === "Enter" && savedFilterName.trim()) {
                                setSavedFilters((p) => [
                                  ...p,
                                  {
                                    ...
                                    status: draftInvStatus,
                                    wdStatus: draftWdStatus,
                                  },
                                ]);
                                setSavedFilterName("");
                        ...
                        <>
                          ...
                          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                            <p className="text-xs text-slate-500">
                              ...
                              fatura{invoiceTotal !== 1 ? "s" : ""} no total
                            </p>
                          </div>
                        </>
                      )}
                        ...
                        <>
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                <button
                                  key={opt.value}
                                  onClick={() => setDraftExpStatus(opt.value)}
                                  className={cn(
                                    "px-3 py-1.5 rounded-full border text-xs font-medium transition-colors",
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                <button
                                  key={opt.value}
                                  onClick={() => setDraftExpType(opt.value)}
                                  className={cn(
                                    "px-3 py-1.5 rounded-full border text-xs font-medium transition-colors",
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {["all", ...EXP_CATEGORIES].map((cat) => (
                                <button
                                  key={cat}
                                  onClick={() => setDraftExpCategory(cat)}
                                  className={cn(
                                    "px-3 py-1.5 rounded-full border text-xs font-medium transition-colors",
                          ...
                          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                            <p className="text-xs text-slate-500">
                              ...
                              despesa{expenseTotal !== 1 ? "s" : ""} no total
                            </p>
                          </div>
                        </>
                      )}
                        ...
                        <>
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                <button
                                  key={opt.value}
                                  onClick={() => setDraftWalletType(opt.value)}
                                  className={cn(
                                    "px-3 py-1.5 rounded-full border text-xs font-medium transition-colors",
                            ...
                            <div className="flex flex-wrap gap-2">
                              {[
                                ...
                                <button
                                  key={opt.value}
                                  onClick={() =>
                                    setDraftWalletStatus(opt.value)
                                  }
                                  className={cn(
                                    "px-3 py-1.5 rounded-full border text-xs font-medium transition-colors",
                          ...
                          <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                            <p className="text-xs text-slate-500">
                              ...
                              carteira{walletTotal !== 1 ? "s" : ""} no total
                            </p>
                          </div>
                        </>
                      )}
            ...
            <StandardModalDialog
              ...
              footer={
                <div className="flex items-center justify-end gap-3">
                  <button
                    onClick={() =>
                      setInvVisibleCols(new Set(DEFAULT_INVOICE_VISIBLE_COLS))
                    }
                  ...
                  <button
                    onClick={() =>
                      setInvVisibleCols(
                        new Set(INVOICE_COLUMNS.map((c) => c.key)),
                      )
                    }
            ...
            <StandardModalDialog
              ...
              footer={
                <div className="flex justify-end gap-2">
                  <Button
                    variant="outline"
                    onClick={() => setExpSheetOpen(false)}
                  >
                    Cancelar
                  </Button>
                  <Button
                    className="btn-brand border-0"
                    disabled={actionLoading === "exp-save"}
                    onClick={handleSaveExpense}
                  >
                    {actionLoading === "exp-save"
                      ? "Salvando…"
                      : editingExpense
                        ? "Salvar"
                        : "Criar Despesa"}
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="max-w-3xl mx-auto space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      placeholder="Ex: Plano Vercel Pro"
                      className="h-9 text-sm"
                      value={expForm.name}
                      onChange={(e) =>
                  ...
                  {(expForm.recurrence === "mensal" ||
                    expForm.recurrence === "anual") &&
                    !editingExpense && (
                      <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-900/40">
                        <input
                          ...
                          onChange={(e) =>
                            setExpForm((f) => ({
                              ...f,
                              is_recurring_base: e.target.checked,
                            }))
                          }
                        />
                        <label
                          htmlFor="recurring-base"
                          className="text-xs text-blue-800 dark:text-blue-300 cursor-pointer"
                        >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        type="month"
                        className="h-9 text-sm"
                        value={expForm.competence_month}
                        onChange={(e) =>
                          setExpForm((f) => ({
                            ...f,
                            competence_month: e.target.value,
                          }))
                        }
                      />
                    </div>
            ...
            <Sheet open={ledgerOpen} onOpenChange={setLedgerOpen}>
              <SheetContent className="w-full sm:max-w-2xl overflow-y-auto p-0 flex flex-col">
                ...
                <div className="app-brand-header px-6 py-4 shrink-0">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-blue-200 text-[10px] font-semibold uppercase tracking-wider mb-0.5">
                        Extrato da Carteira
                      </p>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        >
                          {WALLET_OWNER_LABELS[selectedWallet?.owner_type] ||
                            selectedWallet?.owner_type}
                        </Badge>
                    ...
                    <div className="text-right">
                      ...
                      {(selectedWallet?.blocked_balance ?? 0) > 0 && (
                        <p className="text-amber-300 text-[10px] mt-0.5">
                          {fmt(selectedWallet?.blocked_balance)} bloqueado
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {ledgerSummary && (
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      {[
                        {
                          label: "Créditos",
                          value: ledgerSummary.credits,
                          color: "text-emerald-300",
                        },
                        {
                          label: "Débitos",
                          value: ledgerSummary.debits,
                          color: "text-red-300",
                        },
                        {
                          label: "Líquido",
                ...
                <div className="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2 flex-wrap shrink-0 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <div className="ml-auto flex items-center gap-1">
                    <button
                      onClick={() => loadLedger(selectedWallet?.id)}
                      className="h-7 w-7 rounded flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                      title="Atualizar"
                    >
                    ...
                    <button
                      onClick={() => {
                        setAdjustWallet(selectedWallet);
                        setAdjustForm({
                          direction: "credit",
                          amount: "",
                          description: "",
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      {ledger.map((entry: any) => {
                        ...
                        return (
                          <div
                            key={entry.id}
                            className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                          >
                            <div className="flex items-start gap-3">
                              {/* Icon */}
                              <div
                                className={`mt-0.5 rounded-full p-1.5 shrink-0 ${isCredit ? "bg-emerald-50 dark:bg-emerald-950/30" : "bg-red-50 dark:bg-red-950/30"}`}
                              >
                                {isCredit ? (
                                  <ArrowUpCircle className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                                ) : (
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <p
                                    className={`text-sm font-bold tabular-nums shrink-0 ${isCredit ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"}`}
                                  >
                                    {isCredit ? "+" : "−"}
                                    {fmt(entry.amount)}
                                  </p>
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className={`text-[9px] py-0 px-1 ${entry.status === "confirmed" ? "text-emerald-600 border-emerald-200" : entry.status === "pending" ? "text-amber-600 border-amber-200" : "text-slate-400 border-slate-200"}`}
                                  >
                ...
                <div className="shrink-0 px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
                  <p className="text-xs text-slate-400">
                    {ledgerTotal} movimentação{ledgerTotal !== 1 ? "ões" : ""}
                  </p>
              ...
              <SheetContent className="w-full sm:max-w-2xl overflow-y-auto p-0 flex flex-col">
                <div className="app-brand-header px-6 py-4 shrink-0">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-blue-200 text-[10px] font-semibold uppercase tracking-wider mb-0.5">
                        Movimentações — Todas as Carteiras
                      </p>
                      <h2 className="text-white font-bold text-lg leading-tight">
                        {globalLedgerTitle || "Extrato Global"}
                      </h2>
                    </div>
                    <div className="text-right">
                      <p className="text-blue-200 text-[10px]">
                        Total (
                        {globalLedgerSummary?.credits >= 0
                          ? "crédito"
                          : "débito"}
                        )
                      </p>
                      {globalLedgerDir === "debit" ? (
                        <p className="text-white font-bold text-2xl tabular-nums">
                          {fmt(globalLedgerSummary?.debits ?? 0)}
                        </p>
                      ) : (
                        <p className="text-white font-bold text-2xl tabular-nums">
                          {fmt(globalLedgerSummary?.credits ?? 0)}
                        </p>
                      )}
                    </div>
                  </div>
                  {globalLedgerSummary && (
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      {[
                        {
                          label: "Créditos",
                          value: globalLedgerSummary.credits,
                          color: "text-emerald-300",
                        },
                        {
                          label: "Débitos",
                          value: globalLedgerSummary.debits,
                          color: "text-red-300",
                        },
                        {
                          label: "Líquido",
                          value: globalLedgerSummary.net,
                          color:
                            globalLedgerSummary.net >= 0
                              ? "text-emerald-300"
                              : "text-red-300",
                        },
                      ].map(({ label, value, color }) => (
                ...
                <div className="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2 flex-wrap shrink-0 bg-slate-50 dark:bg-slate-800/50">
                  <span className="text-[11px] text-slate-500">
                    {globalLedgerTotal} movimentação
                    {globalLedgerTotal !== 1 ? "ões" : ""}
                  </span>
                  <button
                    onClick={() => loadGlobalLedger()}
                    className="ml-auto h-7 w-7 rounded flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                    title="Atualizar"
                  >
                    <RefreshCw className="h-3.5 w-3.5" />
                  </button>
                  ...
                  {globalLedgerLoading ? (
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      {globalLedger.map((entry: any) => {
                        const isCredit = entry.direction === "credit";
                        const ownerLabel =
                          WALLET_OWNER_LABELS[entry.wallet?.owner_type] ||
                          entry.wallet?.owner_type ||
                          "—";
                        const ownerColor =
                          WALLET_OWNER_COLORS[entry.wallet?.owner_type] || "";
                        return (
                          <div
                            key={entry.id}
                            className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                  ...
                                  {entry.wallet?.owner_name && (
                                    <span
                                      className={`text-[10px] px-1.5 py-0.5 rounded border ${ownerColor}`}
                                    >
                ...
                <div className="shrink-0 px-4 py-2.5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
                  <p className="text-xs text-slate-400">
                    {globalLedgerTotal} mov.
                  </p>
                  <div className="flex items-center gap-0.5">
                    <button
                      onClick={() =>
                        setGlobalLedgerPage((p) => Math.max(1, p - 1))
                      }
                      disabled={globalLedgerPage === 1}
                      className="h-7 w-7 rounded flex items-center justify-center disabled:opacity-30 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 transition-colors"
                    >
                      <ChevronLeft className="h-4 w-4" />
                    </button>
                    <span className="text-xs text-slate-400 px-1">
                      {globalLedgerPage} / {globalLedgerTotalPages}
                    </span>
                    <button
                      onClick={() =>
                        setGlobalLedgerPage((p) =>
                          Math.min(globalLedgerTotalPages, p + 1),
                        )
                      }
                      disabled={globalLedgerPage === globalLedgerTotalPages}
                      className="h-7 w-7 rounded flex items-center justify-center disabled:opacity-30 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 transition-colors"
                    >
                      <ChevronRight className="h-4 w-4" />
                    </button>
            ...
            <Sheet open={projectionsOpen} onOpenChange={setProjectionsOpen}>
              <SheetContent className="w-full sm:max-w-xl overflow-y-auto p-0 flex flex-col">
                <div className="app-brand-header px-6 py-4 shrink-0">
                  <p className="text-blue-200 text-[10px] font-semibold uppercase tracking-wider mb-0.5">
                    Projeções — próximos {walletHorizon} dias
                  </p>
                  <h2 className="text-white font-bold text-lg">
                    {projectionsMode === "credits"
                      ? "Créditos futuros previstos"
                      : "Débitos futuros previstos"}
                  </h2>
                  <p
                    className={`text-2xl font-bold tabular-nums mt-1 ${projectionsMode === "credits" ? "text-emerald-300" : "text-orange-300"}`}
                  >
                    {fmt(
                      projectionsMode === "credits"
                        ? (walletProjections?.futureCredits ?? 0)
                        : (walletProjections?.futureDebits ?? 0),
                    )}
                  </p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                  {projectionsMode === "credits" ? (
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      {(walletProjections?.pendingInvoices || []).length ===
                      0 ? (
                        <div className="py-16 text-center text-sm text-slate-400 flex flex-col items-center gap-2">
                          <CalendarClock className="h-8 w-8 opacity-30" />
                          <p>
                            Nenhuma fatura pendente nos próximos {walletHorizon}{" "}
                            dias
                          </p>
                        </div>
                      ) : (
                        (walletProjections?.pendingInvoices || []).map(
                          (inv: any) => (
                            <div
                              key={inv.id}
                              className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                            >
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between gap-2">
                                    <p className="text-xs font-medium text-slate-800 dark:text-slate-200 truncate">
                                      {inv.description || "Fatura pendente"}
                                    </p>
                                    <p className="text-sm font-bold text-emerald-600 dark:text-emerald-400 tabular-nums shrink-0">
                                      +{fmt(inv.amount)}
                                    </p>
                                  </div>
                                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800">
                      ...
                      0 ? (
                        ...
                        (walletProjections?.recurringProjects || []).map(
                          (proj: any) => (
                            <div
                              key={proj.id}
                              className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    <p className="text-sm font-bold text-orange-600 dark:text-orange-400 tabular-nums shrink-0">
                                      −{fmt(proj.value)}
                                    </p>
                                  </div>
                                  <div className="flex items-center gap-2 mt-0.5">
                                    ...
                                    <Badge
                                      variant="outline"
                                      className="text-[9px] py-0 px-1 text-emerald-600 border-emerald-200"
                                    >
                                      recorrente
                                    </Badge>
                                  </div>
                ...
                <div className="shrink-0 px-4 py-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex gap-2">
                  <button
                    onClick={() =>
                      setProjectionsMode(
                        projectionsMode === "credits" ? "debits" : "credits",
                      )
                    }
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="max-w-3xl mx-auto space-y-4">
                  {adjustWallet && (
                    <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                      <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                        {adjustWallet.owner_name}
                      </p>
                      <p className="text-[11px] text-slate-400">
                        {WALLET_OWNER_LABELS[adjustWallet.owner_type]} · Saldo
                        atual:{" "}
                        <span className="font-bold text-slate-700 dark:text-slate-300">
                          {fmt(adjustWallet.balance)}
                        </span>
                      </p>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setAdjustForm((f) => ({ ...f, direction: value }))
                          }
                          className={cn(
                            "flex items-center justify-center gap-2 h-10 rounded-lg border-2 text-xs font-semibold transition-colors",
                            adjustForm.direction === value
                              ? color
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      placeholder="Ex: Crédito bônus campanha, Estorno pagamento…"
                      className="h-9 text-sm"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      placeholder="Motivo, referência, contexto…"
                      className="h-9 text-sm"
