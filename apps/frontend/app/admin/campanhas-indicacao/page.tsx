...
const ITEM_COLUMNS: {
  ...
  info: string;
  sortField?: string;
  sortType?: "text" | "number" | "date" | "status";
}[] = [
  {
    key: "nome",
    label: "Nome / Código",
    info: "Nome da campanha ou código do cupom.",
    sortField: "name",
    sortType: "text",
  },
  {
    key: "tipo",
    label: "Tipo",
    info: "Campanha (indicação/link) ou cupom (desconto/bônus/afiliado).",
  },
  {
    key: "status",
    label: "Status",
    info: "Situação atual do item.",
    sortField: "status",
    sortType: "status",
  },
  {
    key: "valor",
    label: "Comissão / Desconto",
    info: "Valor de comissão (campanha) ou desconto/bônus (cupom).",
    sortField: "value",
    sortType: "number",
  },
  {
    key: "vinculado",
    label: "Vinculado a",
    info: "Usuário vinculado a este item, se houver.",
  },
  {
    key: "uso",
    label: "Uso / Indicações",
    info: "Indicações ativas (campanha) ou usos realizados (cupom).",
    sortField: "usage",
    sortType: "number",
  },
  {
    key: "total_pago",
    label: "Total pago",
    info: "Total pago em comissões — aplica-se só a campanhas.",
    sortField: "totalPaid",
    sortType: "number",
  },
  {
...
function couponToUnifiedRow(c: Coupon): UnifiedRow {
  return {
    ...
    status: c.status,
    value:
      c.couponType === "credit-bonus" ? (c.creditBonus ?? 0) : c.discountValue,
    valueLabel:
...
function mapCampaignToApi(form: any) {
  return {
    ...
    status: form.status,
    commission_type:
      form.commissionType === "percentage" ? "percentage" : "fixed",
...
function mapCouponFromApi(c: any): Coupon {
  return {
    ...
    applicableProducts:
      Array.isArray(c.applicable_products) && c.applicable_products.length > 0
...
export default function CampanhasPage() {
  ...
  const [itemPageSize, setItemPageSize] = useItemsPerPage(
  ...
  const [itemKindFilter, setItemKindFilter] = useState<
    Set<"campaign" | "coupon">
  >(new Set());
  const [itemStatusFilter, setItemStatusFilter] = useState<Set<string>>(
    new Set(),
  );
  ...
  const filteredItems = allItems.filter((row) => {
    if (
      itemSearch &&
      !row.name.toLowerCase().includes(itemSearch.toLowerCase())
    )
      return false;
    if (itemKindFilter.size > 0 && !itemKindFilter.has(row.kind)) return false;
    if (itemStatusFilter.size > 0 && !itemStatusFilter.has(row.status))
      return false;
    return true;
  });
        ...
        setCampaigns((prev) =>
          prev.map((c) =>
            c.id === editingCampaign.id
              ? { ...c, ...data, ...mapCampaignFromApi(updated) }
              : c,
          ),
        );
        toast({
  ...
  const saveCoupon = async () => {
    ...
    try {
      if (editingCoupon) {
        const updated = await apiClient.updateCoupon(
          editingCoupon.id,
          mapCouponToApi(data),
        );
        setCoupons((prev) =>
          prev.map((c) =>
            c.id === editingCoupon.id
              ? { ...c, ...data, ...mapCouponFromApi(updated) }
              : c,
          ),
        );
        toast({
  ...
  const totalItemPages = Math.max(
    1,
    Math.ceil(sortedItems.length / itemPageSize),
  );
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="space-y-5">
            ...
            <div className="mt-[5px] grid grid-cols-2 gap-3 lg:h-[65px] lg:grid-cols-4">
              <div className="relative h-full rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 bg-linear-to-br from-blue-500 to-blue-700 dark:from-blue-800 dark:to-blue-950 border-2 border-blue-300/70 dark:border-blue-800/70 px-3 pt-2 pb-1.5">
              ...
              <div className="relative h-full rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 bg-linear-to-br from-emerald-500 to-teal-600 dark:from-emerald-800 dark:to-teal-900 border-2 border-emerald-300/70 dark:border-emerald-800/70 px-3 pt-2 pb-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-white/70 leading-tight">
                    Indicações Ativas
                  </p>
              ...
              <div className="relative h-full rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 bg-linear-to-br from-violet-500 to-purple-700 dark:from-violet-800 dark:to-purple-950 border-2 border-violet-300/70 dark:border-violet-800/70 px-3 pt-2 pb-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-white/70 leading-tight">
                    Usos de Cupons
                  </p>
              ...
              <div className="relative h-full rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-200 bg-linear-to-br from-orange-500 to-rose-600 dark:from-orange-800 dark:to-rose-900 border-2 border-orange-300/70 dark:border-orange-800/70 px-3 pt-2 pb-1.5">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-medium text-white/70 leading-tight">
                    Total Investido
                  </p>
            ...
            <div className="bg-white dark:bg-slate-900 border border-[#e8edf5] dark:border-slate-700 rounded-xl shadow-sm overflow-hidden">
                  ...
                  <IconToolbarButton
                    ...
                    tooltip={
                      itemActiveFilterCount > 0
                        ? `Filtros (${itemActiveFilterCount} ativos)`
                        : "Filtros"
                    }
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 px-[18px] py-2 border-y border-[#e8edf5] dark:border-slate-800 bg-white dark:bg-slate-900/30">
                ...
                {totalItemPages > 1 && (
                  <div className="flex items-center gap-0.5 flex-shrink-0">
                    ...
                    <button
                      onClick={() =>
                        setItemPage((v) => Math.min(totalItemPages, v + 1))
                      }
              ...
              {sortedItems.length === 0 ? (
                ...
                <div
                  ref={itemTableScrollRef}
                  onScroll={handleItemTableScroll}
                  ...
                  <table className="tabela-cartao w-full text-xs min-w-[960px]">
                    <thead>
                      <tr className="border-b border-slate-200/60 dark:border-slate-700/60">
                        ...
                        {ITEM_COLUMNS.filter((c) =>
                          itemVisibleCols.has(c.key),
                        ).map((col) => (
                          <th
                            key={col.key}
                    ...
                    <tbody>
                      {pagedItems.map((row, i) => {
                        const statusTone: Record<string, string> = {
                          ...
                          disabled:
                            "border-rose-500 bg-rose-200 text-rose-900 shadow-[0_0_12px_rgba(244,63,94,0.65)] dark:bg-rose-800/70 dark:text-rose-100",
                          expired:
                            "border-slate-400 bg-slate-300 text-slate-800 shadow-[0_0_8px_rgba(100,116,139,0.4)] dark:bg-slate-800 dark:text-slate-300",
                        };
                        ...
                        return (
                          ...
                          >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <button
                                      onClick={() =>
                                        row.kind === "campaign"
                                          ? openEditCampaign(row.campaign!)
                                          : openEditCoupon(row.coupon!)
                                      }
                                      className="h-[26px] w-[26px] flex items-center justify-center rounded-[8px] bg-white dark:bg-slate-800 border border-[#e8edf5] dark:border-slate-700 text-[#6E2C96] dark:text-slate-500 shadow-[0_4px_10px_rgba(15,23,42,0.06)] hover:bg-gradient-to-br hover:from-[#2558FF] hover:via-[#6E2C96] hover:to-[#D92293] hover:text-white dark:hover:text-[#0a1628] hover:border-transparent hover:shadow-[0_8px_18px_rgba(15,23,42,0.18)] hover:-translate-y-px transition-all duration-150"
                                    >
                                  ...
                                  row.campaign!.campaignType ===
                                    ...
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <button
                                          onClick={() => {
                                            setAccessDrawerCampaign(
                                              row.campaign!,
                                            );
                                            setAccessForm({
                                              name:
                                                row.campaign!.linkedUserName ??
                                                "",
                                              email: "",
                                              password: "",
                                              pixKey: "",
                            ...
                            {itemVisibleCols.has("nome") && (
                              ...
                              >
                                <div className="flex items-center gap-2.5">
                                  <div
                                    className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${row.kind === "campaign" ? "bg-blue-100 dark:bg-blue-900/30" : "bg-violet-100 dark:bg-violet-900/30"}`}
                                  >
                                    {row.kind === "campaign" ? (
                                      row.campaign!.campaignType ===
                                      "influencer" ? (
                                      ...
                                      (() => {
                                        const TypeIcon = getCouponTypeConfig(
                                          row.coupon!.couponType,
                                        ).icon;
                                        return (
                                  ...
                                  <div className="min-w-0">
                                    <p
                                      className={`font-bold text-sm text-slate-800 dark:text-slate-100 ${row.kind === "coupon" ? "font-mono" : ""} truncate`}
                                    >
                                      {row.name}
                                    </p>
                                    <NeonBadge
                                    ...
                                    >
                                      {row.kind === "campaign"
                                        ? (CAMPAIGN_TYPE_LABELS[row.type] ??
                                          row.type)
                                        : getCouponTypeConfig(row.type).label}
                                    </NeonBadge>
                                  </div>
                                </div>
                              </td>
                            ...
                            {itemVisibleCols.has("status") && (
                              ...
                              >
                                <span
                                  className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-bold w-fit border ${statusTone[row.status] ?? statusTone.ended}`}
                                >
                                  <span
                                    className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${statusDot[row.status] ?? statusDot.ended}`}
                                  />
                              ...
                              >
                                {row.totalPaid !== null ? (
                                  <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                    R$ {row.totalPaid.toLocaleString()}
                                  </span>
                                ) : (
                                  <span className="text-slate-300 dark:text-slate-600">
                                    —
                            ...
                            {itemVisibleCols.has("validade") && (
                              <td
                                data-rotulo="Validade"
                                className="py-3 px-4 text-xs text-slate-500 dark:text-slate-400"
                              >
                                {row.validFrom
                ...
                <div className="max-w-3xl mx-auto px-6 py-5 space-y-4">
                      ...
                      <Select
                        value={
                          couponForm.applicableProducts[0] === "Todos os planos"
                            ? "all-plans"
                            : couponForm.applicableProducts[0]?.startsWith(
                                  "Plano",
                                )
                  ...
                  {couponForm.couponType !== "referral" &&
                    (() => {
                      ...
                      const selectedCompanyObjs = PLATFORM_COMPANIES.filter(
                        (co) =>
                          couponForm.allowedCompanyIds.includes(String(co.id)),
                      );
                      // Users that belong to selected companies
                      const usersOfSelectedCompanies = platformUsers.filter(
                        (u) =>
                                ...
                                <div className="pt-2 border-t border-slate-200 dark:border-slate-700 space-y-2">
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <div className="max-h-52 overflow-y-auto p-1">
                                        {filteredUsersForCompany.length ===
                                        0 ? (
                                          <p className="text-xs text-slate-400 text-center py-4">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="space-y-1.5">
                              ...
                              <Input
                                ...
                                placeholder="0"
                                value={
                                  couponForm.linkedUserCommissionValue || ""
                                }
                                onChange={(e) =>
                                  setCouponForm({
                                    ...couponForm,
            ...
            <ConfirmationDialog
              ...
              title={
                couponToggle.newStatus ? "Ativar Cupom" : "Desativar Cupom"
              }
              message={
                    ...
                    <div className="max-w-lg space-y-5">
                      <div className="rounded-xl bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 px-4 py-3 text-sm text-violet-700 dark:text-violet-300">
                        ...
                        <p className="text-xs opacity-80">
                          O parceiro receberá credenciais para acessar o portal
                          em <span className="font-medium">/parceiro</span> e
                          visualizar seus dados de indicação, comissões e
                          solicitar saques.
                        </p>
                      </div>
                ...
                >
                  ...
                  <div className="px-5 py-3">
                    {reportItems.length === 0 ? (
                      ...
                      <table className="tabela-cartao w-full text-xs border-collapse">
                        <thead>
                          ...
                          {reportItems.map((item, idx) => {
                            ...
                            return (
                              ...
                              >
                                <td
                                  data-rotulo="Nome / Código"
                                  className="px-2 py-1.5 font-medium text-slate-700 dark:text-slate-200 max-w-[180px] truncate"
                                >
                                  {item.name}
                                </td>
                                <td
                                  data-rotulo="Tipo"
                                  className="px-2 py-1.5 text-slate-500 dark:text-slate-400"
                                >
                                  {item.kind}
                                </td>
                                <td
                                ...
                                <td
                                  data-rotulo="Conta"
                                  className="px-2 py-1.5 text-slate-400 dark:text-slate-500"
                                >
                                  {item.accountType}
                                </td>
                                <td
                                ...
                                <td
                                  data-rotulo="Usos"
                                  className="px-2 py-1.5 text-right tabular-nums text-slate-600 dark:text-slate-300"
                                >
                                ...
                                <td
                                  data-rotulo="Taxa Conv."
                                  className="px-2 py-1.5 text-right tabular-nums font-semibold text-violet-600"
                                >
                                  {item.rate}%
                                </td>
                                <td
                                ...
                                <td
                                  ...
                                  className="px-2 py-1.5 text-right tabular-nums font-semibold text-amber-600"
                                >
              ...
              footer={
                <div className="flex items-center justify-end gap-3">
                  ...
                  <button
                    onClick={() =>
                      setItemVisibleCols(
                        new Set(ITEM_COLUMNS.map((c) => c.key)),
                      )
                    }
