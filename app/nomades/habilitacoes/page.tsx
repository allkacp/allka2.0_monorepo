...
import { Award, CheckCircle2, XCircle, Clock, BookOpen, ExternalLink, Search } from "lucide-react"
...
const STATUS_CONFIG: Record<string, {
  ...
  certificado:    { label: "Certificado",    badgeCls: "bg-emerald-100 text-emerald-700 border-emerald-200", iconCls: "text-emerald-500", Icon: CheckCircle2 },
  pendente:       { label: "Pendente",       badgeCls: "bg-amber-100 text-amber-700 border-amber-200",       iconCls: "text-amber-500",   Icon: Clock },
  em_andamento:   { label: "Em andamento",   badgeCls: "bg-blue-100 text-blue-700 border-blue-200",          iconCls: "text-blue-500",    Icon: BookOpen },
  revogado:       { label: "Revogado",       badgeCls: "bg-red-100 text-red-700 border-red-200",             iconCls: "text-red-500",     Icon: XCircle },
  nao_habilitado: { label: "Não habilitado", badgeCls: "bg-slate-100 text-slate-500 border-slate-200",       iconCls: "text-slate-300",   Icon: Award },
}
...
export default function NomadesHabilitacoesPage() {
  ...
  const certified   = HABILITACOES.filter((h) => h.status === "certificado").length
  const inProgress  = HABILITACOES.filter((h) => h.status === "pendente" || h.status === "em_andamento").length
  const revoked     = HABILITACOES.filter((h) => h.status === "revogado").length
  const notEnabled  = HABILITACOES.filter((h) => h.status === "nao_habilitado").length

  const filtered = HABILITACOES.filter((h) => {
    const q = search.toLowerCase()
    const matchSearch = h.type.toLowerCase().includes(q) || h.category.toLowerCase().includes(q)
    const matchTab =
      tab === "todos" ||
      (tab === "certificado" && h.status === "certificado") ||
      (tab === "em_andamento" && (h.status === "pendente" || h.status === "em_andamento")) ||
      (tab === "outros" && (h.status === "nao_habilitado" || h.status === "revogado"))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Certificadas"  value={certified}  color="emerald" icon={CheckCircle2} />
        <StatCard label="Em andamento"  value={inProgress} color="blue"    icon={Clock} />
        <StatCard label="Revogadas"     value={revoked}    color="red"     icon={XCircle} />
        <StatCard label="Não habilitado" value={notEnabled} color="slate"  icon={Award} />
      </div>
      ...
      <div className="flex items-center gap-4 rounded-xl bg-linear-to-r from-violet-50 to-blue-50/60 border border-violet-200 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Complete cursos e testes para ampliar seu catálogo de tarefas disponíveis.
          </p>
        </div>
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 truncate">{h.type}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{h.category}</p>
                </div>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full border shrink-0 ${cfg.badgeCls}`}>
                  {cfg.label}
                </span>
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  {h.status === "certificado" && (
                    <>
                      <p className="text-xs text-slate-500">Certificado em {h.certifiedAt}</p>
                      {h.score && (
                        <p className="text-xs font-semibold text-emerald-600 mt-0.5">Pontuação: {h.score}/100</p>
                      )}
                      {h.expiresAt && (
                        <p className="text-xs text-slate-400 mt-0.5">Válido até {h.expiresAt}</p>
                      )}
                    </>
                  )}
                  {h.status === "pendente" && (
                    <p className="text-xs text-slate-500">Teste agendado para {h.testDate}</p>
                  )}
                  {h.status === "em_andamento" && (
                    <div className="w-full">
                      <p className="text-xs text-slate-500 mb-1.5">Progresso do curso: {h.progress}%</p>
                  ...
                  {h.status === "revogado" && (
                    <>
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">Revogado em {h.revokedAt}</p>
                    </>
                  )}
                  {h.status === "nao_habilitado" && (
                    <p className="text-xs text-slate-400">Ainda não certificado nesta área</p>
                  )}
                </div>
              </div>
              ...
              {h.status !== "certificado" && (
                <Link to={h.allkademyPath}>
                  <Button variant="outline" size="sm" className="w-full gap-1.5 text-xs h-8">
                    ...
                    {h.status === "revogado" ? "Recertificar na Allkademy" : "Ir para Allkademy"}
                  </Button>
                </Link>
              )}
            </div>
