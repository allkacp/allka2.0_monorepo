...
export default function AdminFinanceiroPage() {
  ...
  const MOCK_PENDING_WITHDRAWALS = [
    {
      ...
      partnerEmail: "carlos@exemplo.com",
      amount: 800,
      pixKey: "carlos@exemplo.com",
      pixKeyType: "email",
      requestedAt: "2025-06-01T14:30:00Z",
      status: "pending" as const,
      notes: "",
    },
    {
      id: "w2",
      partnerName: "Ana Beatriz",
      partnerEmail: "ana.beatriz@nomade.io",
      amount: 450,
    ...
    {
      id: "w3",
      partnerName: "Pedro Alves",
      partnerEmail: "pedro@agencia.com",
      amount: 1200,
      pixKey: "11987654321",
      pixKeyType: "phone",
      requestedAt: "2025-05-30T16:00:00Z",
      status: "pending" as const,
      notes: "",
    },
  ];
  ...
  const [withdrawals, setWithdrawals] = useState<Array<{
    id: string; partnerName: string; partnerEmail: string; amount: number;
    pixKey: string; pixKeyType: string; requestedAt: string;
    status: "pending" | "paid" | "rejected"; notes: string;
  ...
  function approveWithdrawal(id: string) {
    setWithdrawals((ws) =>
      ws.map((w) =>
        w.id === id
          ? { ...w, status: "paid" as const, notes: "Aprovado e pago" }
          : w,
      ),
    );
  }
  ...
  function rejectWithdrawal(id: string) {
    setWithdrawals((ws) =>
      ws.map((w) =>
        w.id === id
          ? { ...w, status: "rejected" as const, notes: rejectNotes[id] ?? "" }
          : w,
      ),
    );
    setRejectingId(null);
