...
export function useCompanyData() {
  ...
  const [clientsByCompany, setClientsByCompany] = useState<Record<number, MockClientItem[]>>({})
  const [usersByCompany, setUsersByCompany] = useState<Record<number, MockUserItem[]>>({})
  ...
  return { companies, clientsByCompany, usersByCompany, loading, refetch: fetchAll }
}
