...
export function adaptApiProject(api: any): FrontendProject {
  const activeStatuses = ["draft", "negotiation", "awaiting-payment", "planning", "in-progress"]
  ...
  return {
    ...
    billingConfig:
      api.billing_day != null
        ? { billingDay: api.billing_day, billingStartDate: api.billing_start_date || "" }
        : undefined,
    companyId: api.client_id || "",
  }
}

export function adaptProjectToApi(project: Partial<FrontendProject>): Record<string, any> {
  ...
  if (project.companyType !== undefined) data.company_type = project.companyType
  if (project.consultant !== undefined) data.consultant = project.consultant
  if (project.consultantEmail !== undefined) data.consultant_email = project.consultantEmail
  if (project.team !== undefined) data.team_size = project.team
  if (project.nomades !== undefined) data.nomades = JSON.stringify(project.nomades)
  if (project.bitrixSync !== undefined) data.bitrix_sync = project.bitrixSync
  if (project.portfolioPermission !== undefined) data.portfolio_permission = project.portfolioPermission
  ...
  if (project.startDate) data.start_date = new Date(project.startDate).toISOString()
  if (project.deadline) data.end_date = new Date(project.deadline).toISOString()
  if (project.companyId !== undefined) data.client_id = String(project.companyId)
