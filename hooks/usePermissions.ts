...
export function usePermissions() {
  ...
  const setPermissions = useCallback(async (profileId: string, permissions: any[]) => {
    await apiClient.updateProfilePermissions(profileId, permissions)
    await fetchProfiles()
  }, [fetchProfiles])

  return { profiles, loading, error, refetch: fetchProfiles, createProfile, updateProfile, deleteProfile, setPermissions }
}
