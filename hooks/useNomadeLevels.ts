...
export function useNomadeLevels() {
  ...
  const createLevel = useCallback(async (data: any) => {
    const result: any = await apiClient.createNomadeLevel(data)
    await fetchLevels()
    return result
  }, [fetchLevels])

  const updateLevel = useCallback(async (id: string, data: any) => {
  ...
  return { levels, loading, error, refetch: fetchLevels, createLevel, updateLevel, deleteLevel }
}
