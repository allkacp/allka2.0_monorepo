...
export function useSpecialties(filters?: Record<string, any>) {
  ...
  const createSpecialty = useCallback(async (data: any) => {
    const result: any = await apiClient.createSpecialty(data)
    await fetchSpecialties()
    return result
  }, [fetchSpecialties])

  const updateSpecialty = useCallback(async (id: string, data: any) => {
  ...
  return { specialties, total, loading, error, refetch: fetchSpecialties, createSpecialty, updateSpecialty, deleteSpecialty }
}
