...
export function useNomades(filters?: Record<string, any>) {
  ...
  useEffect(() => { fetchNomades() }, [fetchNomades])

  const createNomade = useCallback(async (data: any) => {
    const result: any = await apiClient.createNomade(data)
    await fetchNomades()
    return result
  }, [fetchNomades])

  const updateNomade = useCallback(async (id: string, data: any) => {
    const result: any = await apiClient.updateNomade(id, data)
    await fetchNomades()
    return result
  }, [fetchNomades])

  const deleteNomade = useCallback(async (id: string) => {
    await apiClient.deleteNomade(id)
    await fetchNomades()
  }, [fetchNomades])

  return { nomades, total, loading, error, refetch: fetchNomades, createNomade, updateNomade, deleteNomade }
}
