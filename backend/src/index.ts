...
async function logStartupState(): Promise<void> {
  ...
  if (products === 0) {
    ...
    console.warn("     Execute: cd backend && npx tsx seed-all-products.ts");
  }
  if (catalogTasks === 0) {
    console.warn("  ⚠️  ATENÇÃO: Nenhum modelo de tarefa no banco.");
    console.warn("     Execute: cd backend && npx tsx migrate-tasks.ts");
  }
  if (projectTasks === 0 && projects > 0) {
    console.warn("  ⚠️  ATENÇÃO: Projetos sem tarefas operacionais.");
    console.warn("     Execute: cd backend && node seed-in-progress.cjs");
  }
  console.log("─────────────────────────────────────────────");
}
