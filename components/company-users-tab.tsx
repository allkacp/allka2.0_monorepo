            ...
            <Popover>
              ...
              <PopoverContent align="end" sideOffset={8} className="w-[220px] p-0">
                  ...
                  {USER_COLS.map(col => (
                    <label
                      ...
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
                        visibleCols.has(col.key) ? "bg-blue-50" : "hover:bg-slate-50"
                      } ${col.required ? "opacity-60 pointer-events-none" : ""}`}
                    >
                      <Checkbox
                        checked={visibleCols.has(col.key)}
                        onCheckedChange={() => !col.required && toggleUserCol(col.key)}
                ...
                <div className="px-4 py-2.5 border-t border-slate-100 flex items-center justify-between">
                  <button
                    onClick={() => setVisibleCols(new Set(USER_COLS.map(c => c.key)))}
                    className="text-[10px] font-medium text-blue-500 hover:text-blue-700 transition-colors"
                  >
                    Mostrar todas
                  </button>
                  <span className="text-[10px] text-slate-400">{visibleCols.size} de {USER_COLS.length}</span>
                </div>
              </PopoverContent>
            </Popover>
