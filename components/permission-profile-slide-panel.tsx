...
import {
  X, Shield, Users, Building2, Briefcase, DollarSign, Settings,
  Compass, Store, BarChart3, FileText, Lock, LayoutDashboard,
  Minus, ChevronDown,
} from "lucide-react"
...
interface FormData   { name: string; description: string; permissions: Record<string, ModulePerm> }

const EMPTY: ModulePerm = { view: "none", create: "none", edit: "none", delete: "none" }
const FULL:  ModulePerm = { view: "all",  create: "all",  edit: "all",  delete: "all"  }
...
// ─── Scope Cell ───────────────────────────────────────────────────────────────
function ScopeCell({ value, onChange }: { value: Scope; onChange: (v: Scope) => void }) {
  return (
    <button
      type="button"
...
function ColCycleBtn({ color, onClick }: { color: string; onClick: () => void }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="top" className="text-[11px] bg-slate-800 text-white border-slate-700">
...
export function PermissionProfileSlidePanel({ open, onClose, profile, onSave }: PermissionProfileSlidePanelProps) {
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (profile) {
      ...
      return { name: profile.name || "", description: profile.description || "", permissions: perms }
    }
  ...
  const setScope = (module: string, action: keyof ModulePerm, scope: Scope) => {
    setFormData((prev) => ({
      ...prev,
      permissions: { ...prev.permissions, [module]: { ...prev.permissions[module], [action]: scope } },
    }))
  }
  ...
  const cycleColumn = (action: keyof ModulePerm) => {
    const allFull = MODULES.every((m) => (formData.permissions[m.key]?.[action] ?? "none") === "all")
    const next: Scope = allFull ? "none" : "all"
    setFormData((prev) => {
      const perms = { ...prev.permissions }
      MODULES.forEach((m) => { perms[m.key] = { ...perms[m.key], [action]: next } })
  ...
  const cycleRow = (moduleKey: string) => {
    ...
    setFormData((prev) => ({ ...prev, permissions: { ...prev.permissions, [moduleKey]: next } }))
  }

  const summary = useMemo(() => {
    let active = 0; let total_all = 0
  ...
  return (
    <TooltipProvider>
      <div
        ...
        className={`fixed top-0 bottom-8 right-0 bg-white shadow-2xl z-50 flex flex-col transition-all duration-300 ease-out ${
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                ...
                {summary.total_all > 0 && ` · ${summary.total_all}× acesso total`}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-5 py-3 border-b bg-slate-50 shrink-0">
          <div className="flex gap-3">
            <div className="flex-1">
              <Label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide">Nome do Perfil</Label>
              <Input
                value={formData.name}
        ...
        <div className="flex items-center flex-wrap gap-x-4 gap-y-1 px-5 py-2 border-b bg-white shrink-0">
          <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mr-1">Escopo:</span>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-slate-500">Somente registros do próprio usuário</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">
              Todos
            </span>
            <span className="text-[11px] text-slate-500">Todos os registros da plataforma</span>
          </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="sticky top-0 z-10">
              <tr style={{ background: "#0f172a" }}>
                <th className="text-left px-4 py-2.5 font-semibold text-xs text-slate-400 border-r border-slate-700" style={{ width: 200 }}>
                  Módulo
                </th>
                {ACTIONS.map((action) => (
                  <th key={action.key} className="text-center px-2 py-2 border-r border-slate-700 last:border-r-0" style={{ width: 130 }}>
                    <div className="flex flex-col items-center gap-1">
                      <span className="text-xs font-semibold text-slate-200">{action.label}</span>
                      <ColCycleBtn color={action.colColor} onClick={() => cycleColumn(action.key)} />
                    </div>
                  </th>
                ))}
              ...
              {GROUPS.map((group) => {
                ...
                return [
                  ...
                  ...groupModules.map((mod, idx) => {
                    ...
                    const rowActive = ACTIONS.filter((a) => perm[a.key] !== "none").length
                    return (
                      <tr
                        key={mod.key}
                      ...
                      >
                        {/* Module label */}
                        <td className="px-4 py-0 border-r border-slate-100" style={{ height: 44 }}>
                          <div className="flex items-center gap-2">
                            <Icon className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                            <span className="text-sm font-medium text-slate-700">{mod.label}</span>
                            {rowActive > 0 && (
                              <span className="ml-auto text-[10px] text-slate-400">{rowActive}/{ACTIONS.length}</span>
                            )}
                          </div>
                        </td>
                        ...
                        {/* Scope cells */}
                        {ACTIONS.map((action) => (
                          <td key={action.key} className="px-2 py-0.5 border-r border-slate-100 last:border-r-0" style={{ height: 44 }}>
                            <ScopeCell
                        ...
                        {/* Row quick-toggle */}
                        <td className="px-2 py-0 text-center" style={{ height: 44 }}>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                type="button"
                            ...
                            <TooltipContent side="left" className="text-[11px] bg-slate-800 text-white border-slate-700">
        ...
        <div className="border-t px-5 py-3 bg-slate-50 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            {summary.total_all > 0 && (
              <Badge variant="outline" className="text-xs text-emerald-700 border-emerald-200 bg-emerald-50 font-normal">
                {summary.total_all}× acesso total
              </Badge>
            )}
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={handleClose} className="h-8 text-sm px-4">
              Cancelar
            </Button>
            <Button
              onClick={() => { onSave(formData); handleClose() }}
              disabled={!formData.name.trim()}
              className="h-8 text-sm px-5 text-white border-0"
              style={{ background: !formData.name.trim() ? undefined : "linear-gradient(135deg, #1e3a8a, #312e81)" }}
            >
              Salvar Perfil
            </Button>
          </div>
